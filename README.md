...
- 단계: **포스트 M5 — UX 고도화·배포 채널 정착**. M0~M5 완료(`0.1.0`~`0.6.0`), 최신 릴리스 **`0.20.0`**(2026-09-08 — 터미널 테마[라이트 팔레트·스킴 15종]·터미널 복사 서식[HTML·RTF]·정규 점검 1차 즉시 조치 6건[덮어쓰기 스테이징·설정 저장 원자화·CFG/CET·플러그인 벽시계]. 직전 `0.19.0` 2026-09-04 — 설정 창 스크롤 재구축·글꼴 폴백 체인·파일 목록 오버레이 스크롤바). 상세 → [docs/STATUS.md](docs/STATUS.md).
- 실측(`0.19.0`): exe **3.85MB**(3.67MiB) ≤10 · 임포트 **OS 인박스 21종만** · 유휴 RSS 16.86MB ≤30(07-15 실측) · 100k 첫 렌더 115ms.
