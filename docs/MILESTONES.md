...
| **포스트 M5** | UX 고도화·배포 채널 정착 — 툴바 SVG·순서 편집 3종 창·컬럼 드래그/auto-fit·다크모드 아이콘·위키 · X-25~32(탭 UX·전송 진행 UX·일본어 언어팩·클립보드/DnD UX) · X-2 **플러그인 wasmi 전환**([ADR-0005](25-adr-0005-wasm-plugins.md)) · X-34 새로 만들기 · X-35 삭제 잠금 처리 · **안정성 감사 7건** · ZIP 자산/SHA256SUMS · **X-36/X-37 클라우드**(Link 탐지 + Connect OAuth 직접 연결 — [ADR-0006](27-adr-0006-cloud-oauth.md)) · X-38 지연 렌더링 드롭 · X-39 보기 범위·탭 DnD · **X-40/X-44 자동 갱신**(프로브 → 셸 통지) · X-41 바 크기 · **X-42 가상 파일 붙여넣기** · X-43 빈 폴더 글리프 · X-45 항상 맨 위 · **X-46 압축 파일 미리보기**([28](28-archive-preview.md)) | B2 **3.83MB**(0.18.1 실측·10진 MB)·B3 통과 21종(클라우드 +4 DLL)·워크스페이스 **320 green** | ✅ `0.7.0`~`0.18.1` — 아래 [포스트 M5 절](#포스트-m5--ux-고도화배포-채널-정착-0700181) 참조. **채널**(09-02 실측): GitHub Release·winget 2채널 **`0.18.1` 라이브** · Chocolatey 2종 **`0.8.1` 승인**(0.18.1 검수 중) |
...
## 포스트 M5 — UX 고도화·배포 채널 정착 (`0.7.0`~`0.18.1`)

> M5 마감 이후는 **사용자 QA가 다음 작업을 정하는** 구간이다(마일스톤 단위가 아니라 릴리스 단위).
> 시간순 서사 = [DEVLOG](DEVLOG.md)·[journal/](journal/), 사용자용 = 위키 [개발 여정](wiki/개발-여정.md).

- ✅ **`0.7.0`~`0.11.0`**(07-16~07-22) — 배포 2채널(포터블+Inno 설치형)·내 PC 가상 최상위·보기 모드 3종·
  **Nx 컨트롤 라이브러리 14종**·**SVG 아이콘 파이프라인**·일괄 이름 변경 v2·탭 UX·전송 진행 UX·
  일본어 언어팩·클립보드/DnD 4종. **Chocolatey·winget 채널 등록**과 위키 개설.
- ✅ **`0.12.0`**(07-27) — **X-2 플러그인**: Starlark로 구축 후 **wasmi로 교체**([ADR-0005](25-adr-0005-wasm-plugins.md) —
  `.wasm` 단일 아티팩트·fuel 격리) + X-34 새로 만들기 · X-35 휴지통 삭제 실패 무통지 해소.
- ✅ **`0.13.0`**(08-02) — **X-36/X-37 클라우드**: Link(동기화 폴더 탐지·네트워크 0) + Connect(OAuth2 PKCE
  직접 연결·토큰 DPAPI) — 탐색·다운로드·업로드·계정 간 복사. [ADR-0006](27-adr-0006-cloud-oauth.md)·**B3 +4 DLL**.
- ✅ **`0.14.0`**(08-02) — X-38 지연 렌더링 드롭 픽스 2건(재조회 + 같은 볼륨 스테이징) · X-39 보기 옵션
  적용 범위 3택 · 패널 간 탭 DnD.
- ✅ **`0.15.0`·`0.16.0`**(08-11) — **X-40 자동 갱신**(갱신 계기가 OS 통지 하나뿐이던 것을 활성화 갱신 +
  폴더 서명 프로브 폴링으로 확장 — 클라우드·네트워크 경로 대응) · X-41 바 크기 확정(도구 모음 28·퀵 런처 24).
- ✅ **`0.17.0`**(08-23) — **X-42 가상 파일 붙여넣기**(RDP·Outlook·zip 내부 = 경로 없는 소스·워커·undo·DnD) ·
  X-43 빈 폴더 글리프 억제 · **X-44 셸 변경 통지 구독**(자리표시자 생성은 mtime도 통지도 안 남긴다는 실측 →
  이벤트 1선 + 보험 스윕 2선) · X-45 항상 맨 위.
- ✅ **`0.18.0`·`0.18.1`**(08-24) — **X-46 압축 파일 미리보기**(압축을 풀지 않고 목록만 — 포맷 레지스트리·
  ZIP/TAR/CAB/RAR/7z 판정/단일 스트림 · NxGrid 그리드 창 · **암호는 구조로 강제**[[Secret](../crates/nexa-core/src/secret.rs)] ·
  **플러그인 ABI v2** + ISO/ar/cpio 31KB 샘플 — [28](28-archive-preview.md)) · **동봉 플러그인 배포 신설**
  (플러그인이 한 번도 배포된 적 없었다는 실측 → 자산 6종) · `0.18.1` = 설치형 VERSIONINFO 보강으로
  **Defender ML 오탐 해소**.
- 🚧 **실기 QA 대기** — X-40 클라우드 실검증 · X-42~X-46 배포분([TODO](TODO.md) §7).
- 🚧 **미배포(09-04)** — **설정 창 본문 스크롤 재구축**(컨테이너 분리·트랙패드 픽셀·오버레이 바·썸 블링크) ·
  **글꼴 폴백 체인 실적용**(두부 방지 — DW 슬롯·대화상자·미리보기 글리프 런, X-48) · **메인 파일 목록
  오버레이 스크롤바**(세로+가로·축별 독립, X-47 1/2) · 트리 하이라이트 픽스. 실기 QA 뒤 다음 릴리스.
- ✅ **배포 채널**(09-02) — Chocolatey 2종 승인으로 **3채널(Release·winget·choco) 전부 개통**.
  제출은 [채널 제출 규칙](21-distribution.md#릴리스-시-채널-제출-규칙-2026-08-23--사용자-지시상시-규칙)에 따른다.
