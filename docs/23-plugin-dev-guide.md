...
- **독립 미리보기 창(F3 · 도크 우상단 ↗)**: **모달** — 설정 콘솔 폰트(`term_font`)의
  **문자 그리드** + 세로/가로 스크롤(휠·Shift+휠·방향키)·드래그 문자 선택(경계 밖
  상하좌우 자동 스크롤)·**Ctrl+C = rich 복사**(평문 + 모노 RTF — 표/박스 정렬 유지).
  박스 드로잉(`┌─┐│…`)·표는 `disp_width()` 폭 계산과 함께 쓰면 정렬이 보장된다.
  **여기 보이는 그대로가 플러그인의 정답 화면**이다.
- **하단 도크**: 같은 lines를 본문 폰트로 축약 표시 — 휠 세로 스크롤·드래그 선택·
  rich 복사 동일. 넓은 표·다이어그램의 정렬 판정은 독립 창 기준.
