...
| **DR-3** | 배포 | **포터블 단일 exe 단독 채널**(설치 불요·영속물=exe 옆 `data\`). **서명 결정(2026-07-15, M5-3)**: **무서명 유지 확정** — 원본 PKG-4 조사(07-11) 결과 Azure Trusted Signing=한국 개인 불가, 잔여 2안(Store 위임 $19/OV 클라우드 연 $100~400)은 비용 결정이라 원본과 공동 보류. 포터블 단독 채널이라 MSIX 서명 강제 없음 — SmartScreen 경고는 알려진 한계로 감수. **재검토 조건**: 원본 PKG-4에서 인증서 확보 시 release.yml에 서명 단계 추가 | MSIX/setup.exe는 원본 담당 |
