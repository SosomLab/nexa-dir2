...
| --- | --- | --- | --- | --- | --- | --- |
| `feat/m3-ops-transfer` | 2026-07-13 | 2026-07-13 (`844b1e3`) | — | 3 | M3-1 — `nexa-ops` rlib 신설(원본 docs/33·FileOps 이식): transfer 단일 경로(같은 폴더 규칙·충돌 순차·4MB 진행·취소·개별 격리·fast path) + Ctrl+C/X/V·워커·세대 가드·Esc 취소·양쪽 재로드. 실기 순번 복제/무동작·테스트 113 green | [2026-07-13](journal/2026-07-13.md) |
...
- **생성**: 2026-07-13 (분기: main `6938df5`). **커밋**: `ce5e918`(nexa-ops 엔진·테스트 8) → `d06cdba`(앱 배선 — 클립보드·워커·통지·취소) → `ea1299e`(docs 현행화). 병합(`844b1e3`): 2026-07-13. 삭제: CI green 확인 후.
