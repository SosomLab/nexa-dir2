...
| --- | --- | --- | --- | --- | --- | --- |
| `feat/starlark-plugin` | 2026-07-26 | — (실기 QA 후) | — | 9 | **X-2 Starlark 플러그인 시스템 + MarkdownViewerPlugin 샘플**(사용자 지시) — ① S1 시임 + **preview_map 오버라이드**(스크립트 EXTS 기본값 > 설정 재정의 > 폴백) ② S2 `starlark 0.14.2`(DR-8 원장 — B2 +4.24MB, 5.74MB ≤10)·`data\plugins\*.star` 지연 로드·동결 캐시·오류 격리·호스트 API(read_text 대상 파일만·disp_width) ③ **독립 미리보기 창 F3**(콘솔 그리드 — 기준 캔버스·벤치 Shift+F3) ④ samples/markdown-viewer(md+**Mermaid** 순수 Starlark)+E2E ⑤ 가이드 [23](23-plugin-dev-guide.md) ⑥ docs ⑦ **설정 창 플러그인 페이지**(목록+사용 여부 체크 — `plugins_disabled` 영속·내장 면역·즉시 적용) ⑧ **미리보기 UX 6종**(↗ 오버레이·모달 창·도크/창 스크롤·드래그 선택·rich 복사[모노 RTF]·드래그 4방향 자동 스크롤) ⑨ ↗ **이미지 버튼화**(emb:popout SVG·hover sel_bg·pressed accent 38%+1px 오프셋·안 릴리스 발화). 218 green·clippy 0·B3 통과 | [2026-07-26](journal/2026-07-26.md) |
...
| `fix/sort-shift-only` | 2026-07-18 | 2026-07-18 (병합) | 2026-07-18 | 1 | **다중 정렬 Shift 전용 복원**(사용자 재확정 — Ctrl 병행 혼동 회귀). 순번 원문자 상시 표시는 유지·187 green | [2026-07-18](journal/2026-07-18.md) |
| `feat/card-scroll` | 2026-07-18 | 2026-07-18 (병합) | 2026-07-18 | 4 | **카드 스크롤·다중 정렬 개선·컬럼 폭 동기화**(사용자 요청 6건) — ① **NexaCardHost** 뷰포트(그리드 오버레이 바·하단 바 상시·kind별 높이 fitting·하단 5px·ctl 재귀 탐색) ② 정렬 순번 ①② **상시 표시** ③ **컬럼 넓이 동기화**(보기 메뉴 4종 하단·기본 on·영속·off=패널 독립·탭=패널 상속·세션 `panel{i}.colw`[탭별 확장 예약]·DPI 리셋 이후 복원 픽스) ④ Dual/Single file panel(s) 라벨. 실기 on/off/복원 3종·188 green | [2026-07-18](journal/2026-07-18.md) |
| `docs/ctl-api-ref` | 2026-07-18 | 2026-07-18 (병합) | 2026-07-18 | 1 | **ctl 개발자 매뉴얼**(판매용 — 사용자 요청) — overview Quick Start(생성·통지·조회 코드)+공통 인자 표·컨트롤 13문서에 함수/프로퍼티/사용 예 레퍼런스 | [2026-07-18](journal/2026-07-18.md) |
| `feat/imagebtn-fit` | 2026-07-18 | 2026-07-18 (병합) | 2026-07-18 | 3 | **ImageFit + ctl 문서 세트**(사용자 요청 2건) — NxIconButton 이미지 표시 모드(Native 원본/Stretch 맞춤·gdipctx image_size)·docs/ctl 신설(허브+overview+13문서·docs/README 연결) | [2026-07-18](journal/2026-07-18.md) |
| `feat/rename-btn-images` | 2026-07-18 | 2026-07-18 (병합) | 2026-07-18 | 4 | **아이콘 보관 + ± 버튼 raster**(사용자 요청 3건) — 원본 앱 아이콘 4종 보관·리네임 ± PNG 12종 제작(활성/비활성×3크기)·NxIconButton 이미지 모드(gdipctx decode_png·Drop RAII)·32px 임베드 교체. 실기 렌더 | [2026-07-18](journal/2026-07-18.md) |
...
| `feat/nx-grid` | 2026-07-18 | 2026-07-18 (`38f26c9`) | 2026-07-18 | 3 | **NxGrid(14호) + X-23 2차**(사용자 요청 7건·시안 5장) — ① NxGrid: 컬럼 리사이즈·체크 마크 열·**오버레이 스크롤바 세로/가로**(얇은 썸 페이드↔드래그 일반 바 — 픽셀 실측)·**행 선택 파일 목록 규약**(Shift/Ctrl/Ctrl+방향키+Space/Ctrl+A)·**GridOpts**(무헤더·지브라·외곽선·Mark Check/⊖) ② **user32 AV 진범** = 빈 셀 빈 Vec→DrawTextW(런타임 이분) ③ 미리보기 NxGrid(행별 제외)+**프리셋 v2 macOS 시안**([…⌄]=[프리셋들/─/Save/Edit]+건수 동일 행·Save 무캐션 DWM 라운드 팝업 기본 이름 전체 선택·Edit 지브라 목록 행별 ⊖ 스테이징) ④ 메뉴버튼 구분선·텍스트박스 EM_SETSEL·Style.danger. 실기 왕복 QA·186 green | [2026-07-18](journal/2026-07-18.md) |
| `feat/preset-menubutton` | 2026-07-18 | 2026-07-18 (`21c9c48`) | 2026-07-18 | 1 | **프리셋 = `…` 메뉴버튼**(PF 시안) — [이름 NxTextBox][… 메뉴] 한 줄(항목 0 = 저장·1.. = 프리셋 클릭 = 카드 스택 복원·저장 후 메뉴 재생성) + **카드 라벨 우측 정렬**. 실기 저장/재구성/복원 왕복·186 green | [2026-07-17](journal/2026-07-17.md) |
| `fix/ctl-borders` | 2026-07-17 | 2026-07-17 (`011db39`) | 2026-07-17 | 3 | **카드 폼 마감**(사용자 QA·요청 4건) — ① 테두리 위계(동색 배경 구별: 콤보 외곽선·타이틀 필 진하게·비활성 진하게) ② **NxLabel(12호)** + 전 kind **좌측 정렬 라벨 열**(PF·한 줄 복수 쌍) — **라벨 폭 언어별 실측**(한/영 정렬 검증) ③ **NxMenuButton(13호)** `… ⌄` 오버플로 메뉴(NXMB_PICK/GETPICK·팝업 폭 실측). 실기 검증·186 green | [2026-07-17](journal/2026-07-17.md) |
| `feat/x23-card-rename` | 2026-07-17 | 2026-07-17 (`3058d5c`) | 2026-07-17 | 1 | **X-23 카드 재편 1차**(사용자 시안 5장) — 일괄 이름 변경 = **NxGroupCard 카드 스택 = 파이프라인**(타이틀 동작 콤보+⊕⊖ — + 아래 추가·− 삭제·**마지막 1장 불가**)·kind 본문 동적 재구성·지역 id·**실시간 미리보기**·Replace Text/RegEx 분리·Padding 콤보·**Date `${}` 포맷 텍스트+? 도움말**(코어 개정·구식 프리셋 이행)·프리셋 = 카드 복원. 실기 왕복·186 green | [2026-07-17](journal/2026-07-17.md) |
...
| `fix/panel-modes-click` | 2026-07-17 | 2026-07-17 (`merge`) | 2026-07-17 | 1 | X-20 **우 패널 무반응 진범**(사용자 QA) — 0-rect 도크의 `y >= dock.y` 상시 참으로 클릭이 빈 도크에 삼켜짐. h>0 가드(panel+win 3곳)·클릭 주입 검증(선택 1) | [2026-07-16](journal/2026-07-16.md) |
| `fix/panel-modes-splitter` | 2026-07-17 | 2026-07-17 (`merge`) | 2026-07-17 | 1 | X-20 공백 화면 **진범**(사용자 QA) — 싱글 모드의 음수 폭 스플리터를 GDI ETO_OPAQUE가 정규화해 패널/도크 전체 덮어칠. 양수 가드+공유 도크 재도장. PrintWindow 캡처 검증 | [2026-07-16](journal/2026-07-16.md) |
| `feat/panel-modes` | 2026-07-16 | 2026-07-16 (`49061f7`) | 2026-07-16 | 1 | **X-20 싱글/듀얼 패널·정보 모드**(사용자 요청 — 원본 FR-C1): 싱글 패널 = 우 패널 레이아웃만 숨김(**상태 보존** — 복귀 원복)·싱글 정보 = 전폭 공유 도크(활성 추종). **선호값/효과 분리**로 싱글 패널 = 싱글 정보 고정(변경 시 상태바 안내)·듀얼 복귀 시 원복. 메뉴 라디오 4종·영속. 179 green | [2026-07-16](journal/2026-07-16.md) |
| `fix/fontbox-caret` | 2026-07-16 | 2026-07-16 (`3ca2911`) | 2026-07-16 | 1 | fontbox 선택 확정 후 **캐럿 맨 뒤**(EM_SETSEL — `,` 체인 이어 입력 자연)·Enter 경로 재검증(캐럿 14/14) | [2026-07-16](journal/2026-07-16.md) |
| `fix/fontbox-enter` | 2026-07-16 | 2026-07-16 (`261d4d4`) | 2026-07-16 | 1 | fontbox 선택 규칙 개정(**콤마 선입력 시에만 체인** — 없으면 A→B 교체)·**Enter 확정 진범**(모달 펌프 VK_RETURN 가로채기 → `FBM_HAS_DROP` 질의로 드롭 열림 시 컨트롤 통과). 실기 ↓↓+Enter 교체 검증 | [2026-07-16](journal/2026-07-16.md) |
| `fix/fontbox-qa` | 2026-07-16 | 2026-07-16 (`e665ee2`) | 2026-07-16 | 1 | fontbox QA 4건 — 클릭 미반영 **진범 2중**(컨테이너 MA_ACTIVATE·LISTBOX DOWN SetFocus → 킬포커스가 클릭 전 팝업 파괴) 해소·회색 띠(LBS_NOINTEGRALHEIGHT)·**바깥 클릭 닫기**(60ms 감시)·HUD 제거(사용자 확정). 항목0 클릭 재검증 | [2026-07-16](journal/2026-07-16.md) |
| `feat/ctl-fontbox` | 2026-07-16 | 2026-07-16 (`9f88551`) | 2026-07-16 | 1 | **ctl 2호 글꼴 입력 fontbox**(사용자 요청 — WT 피커 참조): 설치 글꼴 드롭다운(각 항목 자기 글꼴 렌더·NOACTIVATE — 포커스 유지)·키보드/마우스 탐색·**쉼표 체인 규칙**(접두사=교체·완결=추가, 단위 테스트)·**커서 위치 타입어헤드 HUD**+매칭 스크롤. prefs 글꼴 6곳 교체·확정=즉시 적용. 실기 검증 | [2026-07-16](journal/2026-07-16.md) |
| `fix/search-clear-zorder` | 2026-07-16 | 2026-07-16 (`722da50`) | 2026-07-16 | 1 | **ctl 커스텀 컨트롤 라이브러리 신설**(사용자 요청) — 1호 `ctl::searchbox` 자기완결 검색박스(컨트롤이 테두리·✕를 자기 영역에 그림+자식 EDIT — ✕ z-순서 진범 구조적 해소)·**세로 중앙 정렬**(한글 상단 붙음)·드롭인 계약(텍스트 위임+EN_CHANGE 재발행). 실기 검증 | [2026-07-16](journal/2026-07-16.md) |
| `fix/font-form-qa` | 2026-07-16 | 2026-07-16 (`f018fe5`) | 2026-07-16 | 1 | 폰트 폼 QA 3건 — **스크롤 공백 진범**(rebuild가 파괴 후 editors 비움 → EN_KILLFOCUS 재진입 harvest가 파괴된 컨트롤서 빈 값 수확 — 순서 교정)·크기 **입력형 콤보**(CBN_SELCHANGE 즉시)·**Enter 즉시 적용**(모달 펌프)·✕ SS_NOTIFY 전환(실클릭 검증) | [2026-07-16](journal/2026-07-16.md) |
| `feat/x12-font-slots` | 2026-07-16 | 2026-07-16 (`148ca3d`) | 2026-07-16 | 1 | **X-12 폰트 슬롯**(사용자 요청) — DwBackend 스타일 포맷 맵(기본/목록[굵게·이탤릭 장식]/상태바·캐시 스타일 비트)·`DrawCtx::select_font`(위젯별 슬롯 선택)·설정 Kind::Font(**이름+크기 1줄**)·폴더 굵게/헤더 굵게·이탤릭·콘솔 폰트 글꼴 카테고리 이동·우클릭 메뉴 폰트 저장(α — HMENU=OS 규약) + **검색박스 내장 ✕ 커스텀**(입력 시만 표시·클릭 전체 지우기 — 실기 검증). 178 green | [2026-07-16](journal/2026-07-16.md) |
| `feat/view-mode-per-tab` | 2026-07-16 | 2026-07-16 (`569ff8e`) | 2026-07-16 | 1 | 보기 모드 **탭별 전환**(사용자 요청) — 활성 탭 전용 적용·새 탭 계승·`panelN.modes` 세션 영속·**도구 모음 라디오 3버튼**([├─][☰][▦] — 1개만)·update_status 단일 동기(탭/패널 전환 자동 반영). 178 green | [2026-07-16](journal/2026-07-16.md) |
| `fix/prefs-group-page` | 2026-07-16 | 2026-07-16 (`41662c0`) | 2026-07-16 | 1 | 설정 그룹 페이지(사용자 QA) — 하위 메뉴 링크 본문 폰트 축소(제목 위계)·그룹 직속 상세 렌더 메커니즘. EnumWindows 실기 검증 | [2026-07-16](journal/2026-07-16.md) |
| `feat/view-modes` | 2026-07-16 | 2026-07-16 (`6d2b75e`) | 2026-07-16 | 1 | 보기 모드 3종(X-19 — 사용자 스크린샷 확정): **타일**(아이콘 32 `L\|` 라지·이름/보조 줄 그리드·러버밴드 rect 교차·그리드 키보드·**내 PC 용량 바**)·**일반 폴더**(펼침 차단 평탄화 — collapse_all 신설)·트리(기존). 내 PC 드라이브 컬럼(전체/여유) 자동 전환·보기 메뉴 라디오·영속. 테스트 4건·177 green | [2026-07-16](journal/2026-07-16.md) |
| `feat/x17-my-pc` | 2026-07-16 | 2026-07-16 (`dbc7077`) | 2026-07-16 | 2 | X-17 가상 최상위 "내 PC"(사용자 요청) — 센티널 `::PC::`+절대 경로 이름(`C:\`)으로 기존 파이프라인 그대로 드라이브 목록·진입·선택·아이콘 동작. nav_up 드라이브 루트→내 PC+떠난 드라이브 선택(G-7)·i18n 표시(내 PC/드라이브)·수정일 빈 셀. 테스트 3건·173 green | [2026-07-16](journal/2026-07-16.md) |
...
| `feat/m5-release-pipeline` | 2026-07-15 | 2026-07-15 (`a491af0`) | 2026-07-15 | 1 | M5-2 릴리스 파이프라인 — release.yml(버전 태그 push→test+예산 게이트[B2·B3]→`NexaDir-<버전>-win-x64.exe`를 GitHub Release 자동 생성·첨부, workflow_dispatch=게이트+아티팩트만)·docs/18 §5 신설(§6 재번호). 릴리스 절차 = `git tag X.Y.Z && git push origin X.Y.Z` | [2026-07-15](journal/2026-07-15.md) |
