...
**압축 완화는 기각(실측 — 하지 말 것)**: 설치형 엔트로피 7.80(포터블 6.48)이 패커
휴리스틱을 자극할까 하여 압축을 낮춰 세 변형을 실제로 빌드·측정했다.

| 설치형 압축 | 크기 | 엔트로피 | 온디맨드 스캔 |
| --- | --- | --- | --- |
| `lzma2/max` + solid(현행) | 3.19MB | 7.80 | clean |
| `lzma` 비솔리드 | 3.20MB | **7.80**(변화 0) | clean |
| `none`(무압축) | **5.76MB** | 7.01 | clean |

**결론 2건**: ① 알고리즘 변경은 엔트로피를 전혀 못 낮추고, 무압축은 **크기 +81%에
엔트로피는 7.01로 임계(≈7.2) 언저리**라 포터블 수준(6.48)에도 못 간다(Inno 셋업 스텁 +
wasm이 자체 엔트로피를 얹는다) — **대가만 크고 오탐 완화는 미미**. ② 애초에 **네 변형
모두 온디맨드 스캔은 clean** = 격리는 *로컬 엔트로피 규칙이 아니라* 다운로드 시점의
**클라우드 평판 판정**(무서명 + 프리밸런스 0 + MOTW)이 유일한 메커니즘. 파일을 만져도
"새 해시 = 평판 0"은 안 바뀐다. ∴ 유효한 것은 **VERSIONINFO(§4-2 1) + 오탐 신고(§4-2 2)
+ 서명/Store(§4-1)**뿐이고 압축은 건드리지 않는다.
