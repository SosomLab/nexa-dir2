...
> **갱신: 2026-09-08 (KST)** — **릴리스 `0.20.0`**(터미널 테마·복사 서식 + 정규 점검 1차 즉시 조치 6건 — 사용자 지시.
...
>
> - **09-10 채널 점검(사용자 요청 — 무변동 정리)**: winget 두 패키지 **`0.20.0` 카탈로그 라이브**(디렉터리 + 로컬 `winget show` —
>   열린 PR 0) · choco `0.20.0` 두 패키지 = 자동 검증(09-09 03:26Z)·verification(13:51Z) 통과 → **스캔 `Pending`**(다음 = 스캔 →
>   `Ready` → 모더레이터. 0.18.1 전례 6일) · `CHOCO_PUSH=true` 유지 · CI 3잡 green · 트리 clean · 코드 무변경.
> - **09-09 Chocolatey `0.18.1` 승인 확인**(모더레이터 virtualex 09-08 — 제출 6일. 0.8.1은 44일) → 규칙상 **`0.20.0`
>   choco 제출 대상 + `CHOCO_PUSH=true` 복원**. 분류기 차단 → 사용자 명시 승인 → 실행: 변수 `true` ·
>   `resubmit-chocolatey` run 성공 → **두 패키지 `0.20.0` `Submitted`**(OData). 그 외 무변동: CI 3잡 green·트리 clean·winget 2채널 `0.20.0` 라이브.
> - **릴리스 `0.20.0`**(09-08 — 사용자 지시 "신규 버전으로 Release"): 채널 실측 = winget `0.19.0` 2건 **09-04 당일
>   병합** → 대기 0건 → **제출** · choco `0.18.1` 두 패키지 **`Ready`**(자동 검증·verification 통과·스캔 1~5 탐지
>   플래그·모더레이터 검토 대기) → **제외**(`CHOCO_PUSH=false` 유지 — push 스텝 `skipped` 확인). CI 3잡 green →
>   태그 → Release run 성공: **자산 6종** · 포터블 exe **3.92MB**(3,923,968B) · 설치형 3.38MB · 해시 3중 일치
>   (SHA256SUMS = GitHub digest = 로컬). winget [#431268](https://github.com/microsoft/winget-pkgs/pull/431268)·[#431269](https://github.com/microsoft/winget-pkgs/pull/431269)
>   제출 → **당일 병합**(`winget validate` 설치형 통과·포터블 경고 2 = 0.19.0과 동일). 릴리스 노트는 앱 페이지용으로 별도 작성
>   (GitHub Release 본문 — 한/영). 위키 5쪽(설정·하단 도크·개발 여정·설치·Home) 갱신·발행.
>
> - **설정 창 깜박임·트랙패드·스크롤바 3건 해소**(`f5b484a`) + 트리 하이라이트 잔존 픽스(`eedd92f`):
>   휠마다 전 컨트롤을 파괴·재생성하던 구조가 진범 → **본문 컨테이너 분리 + 자식 일괄 이동** · 휠
>   잔량 누적 = **픽셀 단위 트랙패드 스크롤** · **오버레이 스크롤바**(반투명·단계 페이드·호버 두꺼움 —
>   사용자 지정 방식) · 더블버퍼로 **썸 블링크 해소**. 교훈 = WS_EX_COMPOSITED 위에선 GDI+ 알파가
>   안 그려진다. 깜박임 소멸 여부는 **배포본 실기 확인 대기**.
> - **메인 파일 목록에도 같은 오버레이 바(세로+가로)** — 사용자 요청·실기 확인. nexa-gui에
>   `Invalidations::request_tick`·`DrawCtx::fill_round_rect_alpha` 일반 경로 신설. 잔여 =
>   NxGrid·카드 호스트 확산([TODO](TODO.md) **X-47** 2/2 — 사용자 결정).
> - **글꼴 폴백 체인 실적용 — "두부 방지"(사용자 요청)**: 체인이 터미널에서만 살아 있고 나머지는
>   **1순위조차 미적용**이던 실측 → [fontchain.rs](../crates/nexa-app/src/fontchain.rs) 신설 ·
>   DW 슬롯 3종 `IDWriteFontFallback` · 미리보기 GDI **글리프 단위 런 폴백** · 대화상자 1순위 정정.
>   네이티브 컨트롤은 OS 글꼴 연결 위임(한계). **글리프 실기 확인 대기 = X-48**.
...
> - **정규 점검 1차(X-51 — 사용자 요청)**: 규격 [29](29-audit-checklist.md)(런북·항목/기준·플러그인·보안·테스트 기법·회차 기록) +
>   [`scripts/audit.ps1`](../scripts/audit.ps1)(실행마다 `docs/audit/<yyyyMMdd-HHmmss>/` 자동 저장) + 벤치·적대적 테스트·커버리지.
>   **결과 원본 [`docs/audit/20260904-165351/`](audit/20260904-165351/README.md)**(회차 보고 §7 위험도별 조치 계획·영역 원본 6·로그).
>   실측 = B1 8.75MB@300s · 기동 430ms · 100k 열거 99ms · VT 15.5MB/s · 커버리지 38.5%(코어 89~97%). **발견 27건(HIGH 14)** → X-52~X-57.
> - **즉시 조치 6건 전부 완료**(사용자 지시 2회): 덮어쓰기 스테이징 교체 · 드롭 거부+원위치 · ConPTY 전체 경로 · CFG/CET/
>   DEPENDENTLOADFLAG+매니페스트 · 플러그인 1,500ms 벽시계+임포트 과금+브레이커 · 설정 저장 원자화+client_secret 직렬화
>   + 테스트가 잡은 4건(VT 2·`get_runs`·압축 이름 절단). 재판정 [`20260904-174325`](audit/20260904-174325/summary.md) PASS 8·WARN 0 · 333 green.
> - **릴리스 뒤 — 터미널 테마(X-49) + 복사 서식(X-50)**(사용자 요청 4건·QA 4라운드 — **미배포**, 재빌드 필요):
>   셀 색 **기호화**(렌더 시 팔레트 해석 = 테마 전환 즉시 스크롤백 재도장) · 라이트 기본 **GitHub Light**
>   (16색 ≥3:1 테스트) · 스킴 15종 · 설정 `term_theme` system/dark/light/스킴 + 모드별 기본(다크 앱 + 라이트
>   스킴 허용) · 선택 = **반전 확정**(라이트 무색 셀만 accent 블록) · **복사 서식 4택**(기본 평문만/HTML/RTF/
>   모두 — nexa-term `export` CF_HTML·RTF) · 설정 창 터미널 하단 드롭다운 4개. 교훈 = **도구 셸 `NO_COLOR`를
>   상속한 앱은 pwsh 색이 꺼진다**(코드 아님 — 재실행 루틴에 env 제거). QA 잔여 = HTML/RTF 붙여넣기·콤보 영속.
>
> ---
>
...
>
> - **09-02** — **배포 채널 마감 + main 적색 복구 + 사용자 문서 3주 공백 보강**: Chocolatey 2종
>   승인(`0.8.1` — 44일) → `CHOCO_PUSH=true` 등록·`0.18.1`만 제출(검수 중) · winget `0.18.1` 라이브 ·
>   `Secret` 소거 테스트의 해제 메모리 읽기 UB 정정으로 **core 잡 9일 적색 복구**(3잡 green) ·
>   위키 타임라인 3주 공백 보강·채널 표 발행 · STATUS 172줄 복원·MILESTONES 포스트 M5 절 신설.
>   [journal/2026-09-02](journal/2026-09-02.md)
...
## 5. 다음 단계 (09-08 기준 — 상세는 [TODO](TODO.md) §7)

1. **점검 1차 발견 처리 순서(09-04 — [29 §9-1](29-audit-checklist.md))**: X-52 파일 조작 HIGH(데이터 손실 2건 최우선) → X-53 설정 저장 원자성 → X-54 렌더/타이머 → X-55 보안(플래그 4줄+pwsh 전체 경로+매니페스트는 즉시 가능) → X-56 플러그인 → X-57 잔여. 코드 서명·OAuth 시크릿·출처 증명은 **사용자 정책 결정**.
2. **실기 QA 잔여분 소화** — 사용자 QA가 병목. 새 기능보다 우선. **최신 대기분 = `0.20.0` 배포분(09-08)** = X-49 터미널 테마 + X-50 복사 서식(라이트 팔레트·스킴 15종·선택자 + 모드별 기본 · HTML/RTF 복사 4택 · 설정 창 터미널 하단 드롭다운 4개. QA 잔여 = Word/Outlook 붙여넣기·콤보 영속·F6 즉시 재색칠) + 즉시 조치 6건 회귀(덮어쓰기 취소 시 원본 보존·전송 중 드롭 거부 안내). **그 앞 = `0.19.0` 배포분 4건**(설정 창 스크롤 깜박임·썸 블링크 소멸 확인 · 파일 목록 오버레이 바 두 축 겹침 · 글꼴 폴백 두부 소멸[X-48] · 트리 하이라이트). **최우선 = X-40 클라우드 실검증**(`0.16.0` 배포됨 — OneDrive/Dropbox/Google Drive 동기화 폴더에서 밖의 저장·동기화가 갱신 버튼 없이 반영되는지. 로컬 NTFS는 기존 watcher로도 잡히므로 **클라우드/네트워크 경로가 진짜 검증 대상**) + **X-42 가상 파일 붙여넣기**(08-23 구현·미배포 — 맥 RDP에서 복사→앱 붙여넣기: 단일/다중/폴더/대용량/한글 이름. Outlook 첨부·zip 내부도 같은 경로) + **X-43 빈 폴더 글리프**(08-23 구현·미배포 — 빈/숨김만 폴더 ▸ 사라짐·보기 토글 복귀·설정 해제 = 현행·네트워크 경로 스크롤 체감. **둘 다 앱 종료 후 재빌드 필요** — 실행 중 exe는 X-41까지). X-41 외관은 3라운드 QA로 확정됨.
...
3. **배포 채널 = 상태 추적**(09-08 실측): **winget `0.19.0` PR 2건 09-04 당일 병합**([#429155](https://github.com/microsoft/winget-pkgs/pull/429155)·[#429156](https://github.com/microsoft/winget-pkgs/pull/429156)) → **`0.20.0` 2건 제출 → 당일 MERGED**([#431268](https://github.com/microsoft/winget-pkgs/pull/431268)·[#431269](https://github.com/microsoft/winget-pkgs/pull/431269) — 시각 = [21 §8](21-distribution.md) 채널 표. winget 2채널 = 최신과 동기) · **Chocolatey 2종 `0.8.1` 승인**(07-20 스캔 플래그 후 44일) → `0.18.1` 두 패키지 제출(09-02) → 09-08 `Ready` → **09-09 `Approved` 확인**(virtualex 09-08 — 6일). → [21 §7·§8](21-distribution.md)
4. **choco `0.20.0` 두 패키지 검수 중**(09-09 02:51Z 제출 [run 34304846007](https://github.com/SosomLab/nexa-dir2/actions/runs/34304846007) → **09-10 실측 = 검증·verification 통과·스캔 `Pending`**) · **`CHOCO_PUSH=true`**(복원 완료) · winget `0.20.0` 두 패키지 09-10 카탈로그 라이브 확인. **다음 릴리스 판정**: winget = 대기 0건 → 제출 · choco = `0.20.0`이 아직 검수 중이면 **`false`로 내려 제외**, `Approved`면 태그 push가 자동 게시. 승인 확인 = `Packages(Id,Version='0.20.0')` 직접 조회.
...
6. **백로그** — **X-47 2/2**(오버레이 바를 NxGrid·카드 호스트로 확산 — 사용자 결정) · X-11 원본 패리티 갭 건별([19](19-parity-gap.md)) · X-2 잔여(공급자 콤보·핫 리로드·비동기 미리보기) · X-16 최적화 잔여 · X-13 2/2(브랜치 `feat/x13-launcher-crud` 보존 중) · X-14·X-15·X-24.
