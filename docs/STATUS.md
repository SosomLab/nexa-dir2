...
> **갱신: 2026-09-04 (KST)** — **설정 창 스크롤 재구축 + 글꼴 폴백 → 릴리스 `0.19.0` → 릴리스 뒤 터미널 테마·복사 서식
> → 정규 점검 1차 + 즉시 조치 6건(전부 미배포)**(사용자 QA·요청 — 상세 [journal/2026-09-04.md](journal/2026-09-04.md)):
>
...
## 5. 다음 단계 (09-04 기준 — 상세는 [TODO](TODO.md) §7)
...
2. **실기 QA 잔여분 소화** — 사용자 QA가 병목. 새 기능보다 우선. **릴리스 뒤 미배포 = X-49 터미널 테마 + X-50 복사 서식**(09-04 — 라이트 팔레트·스킴 15종·선택자 + 모드별 기본 · HTML/RTF 복사 4택 · 설정 창 터미널 하단 드롭다운 4개 — **재빌드 필요**. QA 잔여 = Word/Outlook 붙여넣기·콤보 영속·F6 즉시 재색칠). **최신 대기분 = `0.19.0` 배포분 4건**(설정 창 스크롤 깜박임·썸 블링크 소멸 확인 · 파일 목록 오버레이 바 두 축 겹침 · 글꼴 폴백 두부 소멸[X-48] · 트리 하이라이트). **최우선 = X-40 클라우드 실검증**(`0.16.0` 배포됨 — OneDrive/Dropbox/Google Drive 동기화 폴더에서 밖의 저장·동기화가 갱신 버튼 없이 반영되는지. 로컬 NTFS는 기존 watcher로도 잡히므로 **클라우드/네트워크 경로가 진짜 검증 대상**) + **X-42 가상 파일 붙여넣기**(08-23 구현·미배포 — 맥 RDP에서 복사→앱 붙여넣기: 단일/다중/폴더/대용량/한글 이름. Outlook 첨부·zip 내부도 같은 경로) + **X-43 빈 폴더 글리프**(08-23 구현·미배포 — 빈/숨김만 폴더 ▸ 사라짐·보기 토글 복귀·설정 해제 = 현행·네트워크 경로 스크롤 체감. **둘 다 앱 종료 후 재빌드 필요** — 실행 중 exe는 X-41까지). X-41 외관은 3라운드 QA로 확정됨.
