...
3. **winget 0.16.0 PR 2건 심사 중**(08-11 제출 — 우리 측 조치 불요·상태 추적만): [#415214](https://github.com/microsoft/winget-pkgs/pull/415214) Portable · [#415215](https://github.com/microsoft/winget-pkgs/pull/415215) 설치형(0.8.1 등록 이후 **첫 버전 업데이트**). 카탈로그는 아직 Portable `0.13.0` · 설치형 `0.8.1`. **다음 릴리스부터 [21 §8 채널 제출 규칙](21-distribution.md) 적용**(대기 중이면 제외·없으면 제출 — 08-23 사용자 지시로 상시 규칙화).
4. **Chocolatey 2종 승인 대기**(우리 측 조치 불요·상태 추적만): 0.8.1 **07-20 스캔 플래그 후 무이벤트**(무서명 exe 오탐 — 모더레이터 면제 필요). **후속 버전 push는 채널 제출 규칙에 따라 계속 제외**(0.8.1이 잠겨 있어 이중 큐 회피 — 0.15.0·0.16.0 모두 팩만 수행). 승인 시 `CHOCO_PUSH=true` 등록으로 재개. → [21 §7·§8](21-distribution.md)
