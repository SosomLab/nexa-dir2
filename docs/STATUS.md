...
> **갱신: 2026-08-11 2차 (KST)** — **X-40 외부·클라우드 변경 무갱신 해소 + X-41 도구 모음
> 24×24(사용자 요청 2건 — `fix/x40-refresh-probe` 2커밋, 실기 QA 대기)**:
> ① **원인 4건** — 공통 뿌리는 **갱신 계기가 `ReadDirectoryChangesW` 통지 하나뿐**이었다는
> 것: `WM_ACTIVATEAPP` 핸들러 **부재**(밖에서 저장하고 돌아와도 재열거 계기 없음) ·
> **통지가 오지 않는 경로에 폴백 부재**(RDCW는 로컬 NTFS 규약 — 클라우드 가상
> 드라이브[DriveFS 등]·네트워크는 **구독이 성공해도 통지가 안 올 수 있고**, 그때 watcher는
> 죽은 게 아니라 **조용한** 것이라 07-31 `is_alive` 자가 치유의 **원리적 사각지대**) ·
> **디바운스 무한 연기**(`SetTimer` 재무장이 만료를 미뤄 **동기화가 끝날 때까지 재로드 0**) ·
> 죽은 watcher 치유가 `update_status`(=사용자 상호작용) 의존
> ② **해결 = 통지에 의존하지 않는 두 번째 계기** — [fsprobe.rs](../crates/nexa-app/src/fsprobe.rs)
> 신설(폴더 mtime **O(1)** + 상한 4096 내 1회 열거 서명·`std::fs`만 써 전 플랫폼) +
> `refresh_on_return`(활성화·최소화 복원 — **변경 없으면 재로드도 없다**) +
> `TIMER_FSPOLL` **3s 폴링**(**창 활성 시에만** — 비활성 `KillTimer`로 **유휴 백그라운드
> 0% 규율[DR-2] 유지**·틱마다 `sync_watchers`) + `WATCH_MAX_MS` **1s 연장 상한**
> ③ **X-41** 시안 3택 → 사용자 **C안 확정**(버튼 셀 24×24 축소·아이콘 16×16 유지):
> `tool_h` 26→24 DIP(**퀵 런처 바 공용**)·아이콘 산식 `b.h-10`→`b.h-8`
> ④ 신규 테스트 5 · **106 green(app)·78(gui)·비Windows check 통과·B2 3.49MB·B3 통과** ·
> release 기동 스모크. **클라우드 실검증 = 사용자 QA**(로컬 NTFS는 기존 watcher로도
> 잡히므로 스모크는 무회귀 확인) ⑤ 곁들여 **clippy 게이트가 08-02부터 깨져 있음 발견**
> (`cross_move_tab` 8/7 — 이번 변경 이전 HEAD에서도 재현·CI에는 clippy 없음).
> [journal/2026-08-11.md](journal/2026-08-11.md).
>
> **직전(08-11 1차)** — **저장소 최신화 + 배포 채널 상태 확인(사용자 지시 · 코드 무변경)**:
...
> [#404528](https://github.com/microsoft/winget-pkgs/pull/404528) OPEN(`Needs-Author-Feedback`
> 해제 유지) · **choco 2종 모더레이션 미승인 유지**(07-20 플래그 후 무이벤트).
> 셋 다 우리 측 조치 불요
> ② **`0.14.0` 배포** — 승격 `1eb11b1` → 태그 push → 게이트 전부 통과 →
> [Release 0.14.0](https://github.com/SosomLab/nexa-dir2/releases/tag/0.14.0) **자산 5종**
> (포터블 exe **3.65MB** ≤10 · `SHA256SUMS.txt`) · **choco 팩 success / push skipped 확인**
> (`CHOCO_PUSH` 게이트 의도대로). 내용 = X-38 7-Zip 드롭 픽스 + X-39 보기 옵션 범위·탭 DnD
> ③ **winget Portable 0.14.0 PR 보류** — 같은 패키지 0.13.0 PR이 OPEN이라 중복 제출 회피
...
> ---
>
> **이전 이력 요약 (08-02 1~4차 ~ 07-15)** — 상세는 [DEVLOG](DEVLOG.md)와 각 일자
...
1. **실기 QA 잔여분 소화** — 사용자 QA가 병목. 새 기능보다 우선. **최우선 = X-40 클라우드 실검증**(`fix/x40-refresh-probe` 미병합 — OneDrive/Dropbox/Google Drive 동기화 폴더에서 밖의 저장·동기화가 버튼 없이 반영되는지) + X-41 도구 모음 24×24 외관.
