...
>   플래그·모더레이터 검토 대기) → **제외**(`CHOCO_PUSH=false` 유지). 릴리스 노트는 앱 페이지용으로 별도 작성
>   (GitHub Release 본문 — 한/영). 위키 4쪽(설정·하단 도크·개발 여정·설치) 갱신·발행.
>
...
| B2 exe 크기 | ≤10MB | **3.85MB**(`0.19.0` 릴리스 포터블 실측 3,845,120B — 0.18.1 3.83MB 대비 +10KB) | 09-04 릴리스 자산 |
