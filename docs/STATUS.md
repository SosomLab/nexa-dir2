...
> **M3-6 watcher 완료**(비재귀·300ms 디바운스·무간섭 재로드 — 외부 생성/삭제 자동 반영 실기
> 확인) + 폴더 이동 시 펼침 이월 + 드롭다운 첫 프레임 깨짐 수정(무효 영역 누락).
> 다음: **M3 마감 게이트**(B1 유휴 RSS 재실측[OLE 상주분 확인] → `0.4.0` 태그) → M4 하단 패널.
...
- **M3** 파일 조작 🚧 — **M3-1~6 전 항목 구현 완료**(전송·삭제/이름변경/새로 만들기·Undo/Redo·셸 메뉴·클립보드/DnD·watcher) · 마감 게이트(B1 유휴 재실측·`0.4.0` 태그) 잔여.
- M4 패널 · M5 마감 — ☐.
...
1. ~~M0~~ ✅ `0.1.0` · ~~M1~~ ✅ `0.2.0` · ~~M2~~ ✅ `0.3.0` · ~~M3-1~6~~ ✅ (07-13).
2. **M3 마감 게이트** — B1 유휴 RSS 재실측(10k·유휴 5분 프로토콜 — OLE 상주분 확인) → `0.4.0` 태그.
3. M4 하단 패널(정보·미리보기·터미널). → [02](02-roadmap.md)
