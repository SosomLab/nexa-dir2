# STATUS — Nexa Dir 진행 현황

> **갱신: 2026-08-11 4차 (KST)** — **내용 정리 2차 + 진행사항 최신화(사용자 지시 · 코드 무변경)**:
> 트리 clean·0/0이라 커밋 대기분은 없었고, 대상은 **릴리스가 만들어 낸 문서 지연 5건**.
> 오전 정리에서 "릴리스마다 사용자 문서를 같은 트랜잭션에"라고 적어 놓고 **기능 문서
> 쪽에서 같은 일이 또 났다**(오전엔 버전·설치 페이지만 챙겼다):
> ① **위키가 없는 버튼 크기를 안내** — "26px 바"(07-19 값·오늘만 세 번 더 움직였다) →
> **도구 모음 28/아이콘 20 · 퀵 런처 24/아이콘 16**·두 바 높이 독립 명시
> ② show 그룹에 **폴더 우선 정렬 토글 누락**(08-02 X-39) 보충
> ③ **자동 갱신 설명이 X-40 이전 세대** — "300ms 디바운스·실패 시 F5"만 있어 **변경
> 프로브가 통째로 빠져 있었다**(사용자가 클라우드 자동 갱신의 이유를 알 길이 없었다)
> → 계기 2종·필요한 이유·배터리·**한계 3종** 서술
> ④ **CLAUDE.md·STATUS 다음 단계가 이미 한 일을 "판단 대기"로**(winget 제출·브랜치 병합)
> ⑤ **[21 §8](21-distribution.md) 채널 표** — 카탈로그 버전과 제출 중 PR 분리·choco
> "push 제외 유지" 명시·**0.14.0/0.15.0을 건너뛴 이유**와 **체크섬·제출 방식 규약** 기록.
> **채널 실측**: winget PR 2건 OPEN·`New-Manifest`(검증 대기) · 카탈로그 아직 Portable
> `0.13.0`/설치형 `0.8.1` · choco 무변동.
> ⑥ **위키 발행**(사용자 승인 — `8d76cc4..01178dc` 5파일): 마지막 발행이 **08-02**여서
> 오늘 고친 분이 전부 미발행이었다(08-02의 "발행을 트랜잭션 꼬리에" 교훈이 **9일 만에
> 재발**). **`README.md`는 발행 제외**(위키 페이지가 아니라 폴더 설명서) · **"미발행
> 15건"은 과대 집계**였고 실제 내용 차이는 **5건**(나머지는 CRLF/LF — 해시 비교는 발행
> 판단에 부적절, 다음부터 `git diff --stat`). 발행 후 **로컬 소스 ≡ 발행본** 확인.
> [journal/2026-08-11.md](journal/2026-08-11.md).
>
> **직전(08-11 3차)** — **릴리스 `0.15.0`·`0.16.0` + winget 2건 제출(사용자 지시)**:
> ① **`0.15.0` 배포**(X-40 자동 갱신 + X-41 1라운드 값) → 사용자 실물 QA로 **바 크기가
> 3라운드 만에 갈렸고**(시안 24 → "충분치 않다, 32" → "빠른 실행은 원복·도구 모음은
> 중간") **확정 = 도구 모음 28[아이콘 20] · 퀵 런처 24[아이콘 16]** → **`0.16.0` 재배포**.
> 구조 변경은 **두 바의 높이 분리**(`launch_row_h` — 종전엔 `launch_h = tool_h`라 한쪽만
> 못 키웠다) · **dw.rs 24 래스터 버킷 신설**(고DPI 125%에서 사용 — 잔여 β 앞당겨 해소)
> ② **[Release 0.16.0](https://github.com/SosomLab/nexa-dir2/releases/tag/0.16.0) 자산 5종**
> (포터블 **3.66MB** ≤10 · 설치형 3.26MB · zip 2 · `SHA256SUMS.txt`)
> ③ **winget PR 2건 제출** — [#415214](https://github.com/microsoft/winget-pkgs/pull/415214)
> Portable · [#415215](https://github.com/microsoft/winget-pkgs/pull/415215) **설치형(0.8.1
> 최초 등록 이후 첫 버전 업데이트** — `DisplayVersion` 계속 제외·locale 설명을 현행
> 기능[클라우드·3개 언어]으로 갱신). 체크섬은 `SHA256SUMS.txt` + **자산 재다운로드
> `Get-FileHash`로 이중 확인**
> ④ **choco는 사용자 지시로 제외** — 0.8.1이 모더레이션에 잠겨 있어 **이중 큐를 만들지
> 않는다**. 두 릴리스 모두 팩 success · **push 미실행** 로그 확인
> ⑤ **사용자 문서를 같은 트랜잭션에** — README·위키 3종 `0.16.0`·예산 표 2행 추가
> (오전 정리에서 "릴리스마다 함께 갱신하지 않으면 반복된다"고 적은 것의 이행).
> [journal/2026-08-11.md](journal/2026-08-11.md).
>
> **직전(08-11 2차)** — **X-40 외부·클라우드 변경 무갱신 해소 + X-41 도구 모음
> 24×24(사용자 요청 2건 — `fix/x40-refresh-probe`, main 병합 완료)**:
> ① **원인 4건** — 공통 뿌리는 **갱신 계기가 `ReadDirectoryChangesW` 통지 하나뿐**이었다는
> 것: `WM_ACTIVATEAPP` 핸들러 **부재**(밖에서 저장하고 돌아와도 재열거 계기 없음) ·
> **통지가 오지 않는 경로에 폴백 부재**(RDCW는 로컬 NTFS 규약 — 클라우드 가상
> 드라이브[DriveFS 등]·네트워크는 **구독이 성공해도 통지가 안 올 수 있고**, 그때 watcher는
> 죽은 게 아니라 **조용한** 것이라 07-31 `is_alive` 자가 치유의 **원리적 사각지대**) ·
> **디바운스 무한 연기**(`SetTimer` 재무장이 만료를 미뤄 **동기화가 끝날 때까지 재로드 0**) ·
> 죽은 watcher 치유가 `update_status`(=사용자 상호작용) 의존
> ② **해결 = 통지에 의존하지 않는 두 번째 계기** — [fsprobe.rs](../crates/nexa-app/src/fsprobe.rs)
> 신설(폴더 mtime **O(1)** + 상한 4096 내 1회 열거 서명·`std::fs`만 써 전 플랫폼) +
> `refresh_on_return`(활성화·최소화 복원 — **변경 없으면 재로드도 없다**) +
> `TIMER_FSPOLL` **3s 폴링**(**창 활성 시에만** — 비활성 `KillTimer`로 **유휴 백그라운드
> 0% 규율[DR-2] 유지**·틱마다 `sync_watchers`) + `WATCH_MAX_MS` **1s 연장 상한**
> ③ **X-41** 시안 3택 → 사용자 **C안 확정**(버튼 셀 24×24 축소·아이콘 16×16 유지):
> `tool_h` 26→24 DIP(**퀵 런처 바 공용**)·아이콘 산식 `b.h-10`→`b.h-8`
> ④ 신규 테스트 5 · **106 green(app)·78(gui)·비Windows check 통과·B2 3.66MB·B3 통과** ·
> release 기동 스모크. **클라우드 실검증 = 사용자 QA**(로컬 NTFS는 기존 watcher로도
> 잡히므로 스모크는 무회귀 확인) ⑤ 곁들여 **clippy 게이트가 08-02부터 깨져 있음 발견**
> (`cross_move_tab` 8/7 — 이번 변경 이전 HEAD에서도 재현·CI에는 clippy 없음).
> ⑥ **내용 정리(같은 브랜치)** — 정리 4건 중 **셋이 "바깥으로 보이는 면"의 노후**였고
> 08-02 4차의 같은 진단이 9일 만에 재발했다: **위키가 사실과 반대를 안내**(winget 설치형을
> "심사 중"으로 — 08-06 병합돼 이미 설치 가능. → ✅ 제공 중 + **§4 설치형 절 신설**) ·
> **버전 표기 3주 노후**(README·위키 3종 `0.13.0` → `0.14.0`) · **자산 크기 실측**
> (`gh release view` — 포터블 3.65MB·zip 1.64MB·설치형 3.25MB·zip 2.71MB. 이 과정에서
> **단위 혼용 발견** = 프로젝트는 10진 MB인데 오전에 적은 `3.49MB`는 MiB였다 → 5곳
> **3.66MB로 정정**) · **clippy 게이트 복구**(`cross_move_tab` 인자 8개 → `CrossMove`
> 구조체 — `-D warnings` exit 0). [journal/2026-08-11.md](journal/2026-08-11.md).
>
> **직전(08-11 1차)** — **저장소 최신화 + 배포 채널 상태 확인(사용자 지시 · 코드 무변경)**:
> ① **저장소 = 최신화할 것이 없었다** — 트리 clean · fetch 무변 · **main↔origin 0/0** ·
> 최신 태그·릴리스 `0.14.0`(08-02). 9일간 커밋 무변동(사용자 QA 대기 구간)
> ② **winget 심사 대기 2건이 둘 다 병합됐다(08-06)** — Portable 0.13.0
> [#410978](https://github.com/microsoft/winget-pkgs/pull/410978) **MERGED 00:16 UTC**
> (`Waived-Policy-Test-1.2` — 0.11/0.12와 같은 패턴) · 설치형
> [#404528](https://github.com/microsoft/winget-pkgs/pull/404528) **MERGED 00:46 UTC**
> = 최초 등록 완료(07-19 제출 → 18일). `winget search SosomLab` 실측으로 **두 채널 라이브**
> 확인(설치형 0.8.1 · Portable 0.13.0). **∴ 심사 대기 4건 → choco 2건**
> ③ **남은 것은 심사가 아니라 버전 격차** — Portable `0.14.0` 제출 가능(08-02 보류 조건 해소) ·
> 설치형은 `0.8.1`로 **6버전 뒤짐**. 둘 다 제출 여부는 사용자 판단 대기
> ④ **choco 2종 무변동** — 07-20 02:2x 스캔 Flagged 이후 **22일 무이벤트**(자동 3단계는
> 완료, 사람 검토만 미착수). `CHOCO_PUSH` **미등록 실측 확인**(`gh variable list` exit 0·빈 목록
> — 08-02의 "토큰 권한으로 못 읽음"은 현재 해당 없음). [journal/2026-08-11.md](journal/2026-08-11.md).
>
> **직전(08-02 6차)** — **릴리스 `0.14.0` + 채널 상태 재점검(사용자 지시)**:
> ① **채널 재점검(원천 실측)** — winget Portable 0.13.0
> [#410978](https://github.com/microsoft/winget-pkgs/pull/410978) OPEN(`Azure-Pipeline-Passed`
> + **`Policy-Test-1.2` waiver 대기** — 0.11/0.12와 같은 패턴) · 설치형
> [#404528](https://github.com/microsoft/winget-pkgs/pull/404528) OPEN(`Needs-Author-Feedback`
> 해제 유지) · **choco 2종 모더레이션 미승인 유지**(07-20 플래그 후 무이벤트).
> 셋 다 우리 측 조치 불요
> ② **`0.14.0` 배포** — 승격 `1eb11b1` → 태그 push → 게이트 전부 통과 →
> [Release 0.14.0](https://github.com/SosomLab/nexa-dir2/releases/tag/0.14.0) **자산 5종**
> (포터블 exe **3.65MB** ≤10 · `SHA256SUMS.txt`) · **choco 팩 success / push skipped 확인**
> (`CHOCO_PUSH` 게이트 의도대로). 내용 = X-38 7-Zip 드롭 픽스 + X-39 보기 옵션 범위·탭 DnD
> ③ **winget Portable 0.14.0 PR 보류** — 같은 패키지 0.13.0 PR이 OPEN이라 중복 제출 회피
> (#410978 종결 후 최신 버전 제출) ④ [21 §8](21-distribution.md) 표 저녁 재점검 ·
> CLAUDE.md `0.14.0`. [journal/2026-08-02.md](journal/2026-08-02.md).
>
> **08-02 5차** — **기능의 날 — 7-Zip 드롭 결함 픽스 2건 + 폴더 우선
> 토글 + 보기 옵션 적용 범위 3택 + 패널 간 탭 DnD(사용자 QA 연속 — 스택 5커밋 main 병합)**:
> ① **7-Zip 드롭 수신 결함**(`fix/dnd-delayed-render`) — 1차: 7-Zip은 **지연 렌더링
> 소스**라 드롭 확정 후 GetData(CF_HDROP) **재조회**가 실제 추출을 트리거하는데
> DragEnter 캐시만 써서 빈 임시 폴더(`7zE…`)만 복사되던 결함 → Drop 재조회.
> 2차 QA(4개 중 2~3개만 생존)가 잔여를 드러냄: 비동기 전송이 **소스의 임시 폴더
> 삭제와 경쟁** → Drop 반환 전 같은 볼륨 rename **스테이징 확보**(크기 무관 즉시·
> 지연 렌더링 시그니처에만 발동 = 일반 소스 무영향)
> ② **폴더 우선 정렬 토글 버튼**(`feat/toolbar-folders-first`) — show 블록 3번째
> (숨김→Dot→폴더우선). 시안 4종 브라우저 미리보기 → **사용자 D안(폴더 안 ↑) 확정**·
> 신규 lang 키 0(pref 키 재사용)
> ③ **보기 옵션 적용 범위 3택**(`feat/view-scope-tab-dnd`) — 전체/좌-우 패널(기본)/
> 활성 탭. 설계 = **값의 SSOT를 탭으로**(범위 설정은 전파 폭만 결정 — 모드 전환
> 마이그레이션 없음)·State는 활성 탭 미러·update_status 단일 길목 동기(전환 즉시
> 툴바 추종)·stale 스탬프 수렴(비활성 탭은 가시화될 때만 재열람)·세션 `panel{i}.views`
> 탭당 3비트·툴팁 "라벨 — 범위"
> ④ **패널 간 탭 DnD** — 반대 패널 탭 바 위 = **즉시 교차 이동 + 드래그 이양**(동일
> 패널 재정렬과 같은 "미리 보기 = 실이동")·**ESC = 프레스 시점 스냅샷 원위치 복귀**
> (값 채택은 커밋으로 유예 → 무손실 취소)·마지막 탭 이동 불가.
> 신규 테스트 7(왕복·집합·스테이징·탭별 값·detach/attach·tabbar 이양)·**101 green
> (nexa-app)·78(nexa-gui)·clippy 0·비Windows check 통과**. 실기 QA = 보기 옵션·설정
> 통과, 스택 5커밋 main ff 병합. 잔여 β = 탭 Ctrl 복제 드래그·드래그 시각 피드백·
> 7-Zip 비%TEMP% 작업 폴더 구성의 경쟁. [journal/2026-08-02.md](journal/2026-08-02.md).
>
> ---
>
> **이전 이력 요약 (08-02 1~4차 ~ 07-15)** — 상세는 [DEVLOG](DEVLOG.md)와 각 일자
> [journal/](journal/). 아래는 하루 한 줄 색인이며, **원문은 삭제되지 않고
> journal에 그대로 있다**(STATUS = "지금 상태 한 장" 규약 — [16 §1](16-doc-git-conventions.md)).
>
> - **08-02(1~4차)** — **클라우드 매뉴얼·브랜딩 + 3사 배포 한도 점검**(1차 — [기능-클라우드](wiki/기능-클라우드.md)
>   신설·잘못된 리디렉션 안내 정정·[ADR-0006 §2-4-1](27-adr-0006-cloud-oauth.md)) · **릴리스 `0.13.0`
>   + winget Portable #410978 + 비Windows CI 복구 2건**(2차 — core 잡이 07-27부터 붉던 것 발견·
>   [18](18-build-and-test.md) 절차 보완) · **문서 내용 정리 + 진행사항 최신화**(3차 — STATUS 565→161줄·
>   DR-7 정정·예산 표 갱신). · **사용자 대상 문서 정리 2차 — 루트 README·위키 + 11일치 발행**(4차 — B3 21종 실측·설치 절 신설·기능-클라우드 222줄 발행). [journal/2026-08-02](journal/2026-08-02.md)
> - **08-01** — **클라우드 하루**(8차까지 — `feat/x36-cloud-connections` 22커밋):
>   [26 검토서](26-cloud-integration-study.md) 신설·2단계 분리 권고 → **X-36 Link**(동기화 폴더 탐지·
>   `::PC::` 클라우드 섹션·Cloud 메뉴) → **X-37 Connect**([ADR-0006](27-adr-0006-cloud-oauth.md) 신설·
>   **DR-2 개정 B3 +4**·PKCE 루프백·DPAPI) → 탐색·다운로드·쓰기·3사 완성 →
>   **실사용 QA 17건 전부 진짜 결함**(Google `client_secret`·로딩 무한 5건·계정 간 복사·
>   Dropbox 5건) · client_id **하이브리드 모델 정정**(사용자 지적) · winget 설치형 #404528
>   피드백 처리. **254 green·3.44MB(B2)**. [journal/2026-08-01](journal/2026-08-01.md)
> - **07-31** — **안정성·배포 하루**: 전 소스 **안정성 감사 7건**(watcher 죽음 = OneDrive 간헐
>   무갱신의 유력 원인·conpty 핸들 경쟁·통지 유실 고착·panic 후크) · 포그라운드 양도
>   (연 프로그램이 뒤에 숨던 결함) · **Release ZIP 자산 + SHA256SUMS**(SmartScreen 다운로드
>   차단 완화) · [12 §4-1](12-packaging-single-exe.md) **서명 경로 재조사**(EV 즉시 통과 2024 폐지·
>   Azure 한국 불가 → 완전 해소는 MS Store MSIX뿐) · 3건 main 병합·push · winget Portable
>   0.12.0 **MERGED** 확인. [journal/2026-07-31](journal/2026-07-31.md)
> - **07-27** — **릴리스 `0.12.0`** + winget Portable PR #408280 · **X-35 휴지통 삭제 실패
>   무통지 해소**(사전 잠금 프로브 + `exists()` diff 백스톱·부분 undo·펼친 하위 폴더 감시로
>   M3-6 α 해소) · **X-34 항목 우클릭 "새로 만들기"**(CLSID_NewMenu 호스팅·생성 후 인라인
>   리네임) · `.star`→`.wasm` 표기 정정 · 플러그인 가이드 [24](24-plugin-dev-guide.md) 보강 ·
>   로컬 브랜치 정리. [journal/2026-07-27](journal/2026-07-27.md)
> - **07-26** — **X-2 플러그인 하루**: Starlark 시스템 구축(시임·preview_map·독립 미리보기 창
>   F3·markdown.star 샘플) → 미리보기 UX 6종 → Mermaid **이미지 수준 렌더**·실행 격리 상한 →
>   **런타임 wasmi 전환 결정·구현**([ADR-0005](25-adr-0005-wasm-plugins.md) — `.wasm` 단일 아티팩트·
>   fuel 격리·**B2 5.74→3.18MB**). [journal/2026-07-26](journal/2026-07-26.md)
> - **07-24** — **문서 정리 하루**: STATUS "한 장" 복원(382→175줄) · docs/README 색인 결손 7건 ·
>   CLAUDE.md 13일 노후화 정정 · **[23 macOS·Linux 확장 검토서](23-cross-platform-feasibility.md)**
>   (중립 40.2%·`ctl` 18종이 최대 비용·**DR-1/2/8이 Linux에선 성립 불가** — 권고 = 맥 렌더 스파이크) ·
>   배포 채널 실측 정정 2건 · 저장소·브랜치 정리. [journal/2026-07-24](journal/2026-07-24.md)
> - **07-22** — 릴리스 **`0.10.0`·`0.11.0`**(GitHub 전용) — **X-32 클립보드·DnD UX 4종**
>   (Ctrl+X 흐림·선택 대상 안 붙여넣기·엣지 자동 스크롤·호버 3초 전환) · 컬럼 리사이즈 단독 조절 ·
>   winget Portable `0.11.0` 제출 · **Wiki 설치 페이지 신설**. [journal/2026-07-22](journal/2026-07-22.md)
> - **07-21** — 릴리스 **`0.9.0` 배포 완료**(보류 방침을 choco push만으로 축소·
>   `vars.CHOCO_PUSH` 스위치 도입) · X-31 **일본어 내장 언어팩**(en·ko·ja) ·
>   **X-30 전송 진행 UX 개편** main 병합(세그먼트 바·`transfer_close_ms`·전송 중 잠금)
>   + QA 3건(DEL 휴지통 워커 이관·최소 3px·5색 팔레트) · 배포 채널 실측 점검 ·
>   배포 보류 방침 확정. [journal/2026-07-21](journal/2026-07-21.md)
> - **07-20** — **X-28 탭 바 UX**(우클릭 New Tab·멀티라인 탭·상하 드래그) ·
>   **X-29 도크 텍스트 문자 단위 선택 복사** · X-27 ✅(툴바 hover 통일) ·
>   X-25 1차(`Conflict::Nested`) · **X-26 ③ About 창** · 백로그 3건 등재.
>   exe 1.45MB. [journal/2026-07-20](journal/2026-07-20.md)
> - **07-19** — **툴바 전면 개편**(아이콘 9종 **SVG 단일 파이프라인**·툴팁 i18n) ·
>   **순서/표시 편집 공통 창**(ordereditor + NxOrderTree) · 컬럼 드래그 재배열·
>   auto-fit · 하단 도크 툴바 버튼 · **UI 용어 체계 확정** · 릴리스 **`0.7.0`·`0.8.0`·
>   `0.8.1`** · **GitHub Wiki 신설**(19페이지) · 다크모드 아이콘·exe 리소스 아이콘 ·
>   **배포 채널 등록**(Chocolatey 2종·winget 2종) · 제품명 "2" 제거.
>   [journal/2026-07-19](journal/2026-07-19.md)
> - **07-18** — **NxGrid(ctl 14호)** 신설(오버레이 스크롤바·행 선택 규약·GridOpts) ·
>   **다중열 정렬**(원본 docs/24 §4 이식) · **Segoe MDL2 글리프 통일** ·
>   **ctl base.rs 재사용 리팩터 + 전수 검토**(잠재 크래시 1건 수정) · 카드 스크롤 뷰포트 ·
>   **컬럼 넓이 동기화** 설정 · ctl 판매 매뉴얼(13문서). 188 green.
>   [journal/2026-07-18](journal/2026-07-18.md)
> - **07-17** — **X-22 일괄 이름 변경 v2**(PF 6동작 패리티 — 코어 재작성·Date 엔진) ·
>   X-21 `shell:` 특수 폴더 · **Nx 컨트롤 세트 완성**(GroupCard·ComboBox·CheckBox·
>   TextBox·IconButton·Button·Segmented·Spin) · **렌더 규약 개정**(AA = DrawCtx
>   백엔드만 · GdipCtx = 유일 GDI+ 접점). [journal/2026-07-17](journal/2026-07-17.md)
> - **07-16** — 대규모 하루(병합 브랜치 14개·등재 4건): **X-16 최적화 1차** ·
>   **X-18 배포 2채널**(DR-3 개정 — 포터블+설치형) · **X-17 가상 최상위 "내 PC"** ·
>   **X-19 보기 모드 3종**(타일/일반/트리) · **X-12 폰트 슬롯** ·
>   **ctl 커스텀 컨트롤 라이브러리 신설**(searchbox·fontbox) ·
>   **X-20 싱글/듀얼 패널·정보 모드**. [journal/2026-07-16](journal/2026-07-16.md)
> - **07-15** — **M5 마감 → `0.6.0` 릴리스 발행**(첫 태그 실행 검증) ·
>   **설정 창 전면 개편 3연타**(계층 트리·검색 필터·드릴다운) · M5-1 보완(런처 바·
>   일괄 이름변경 파이프라인) · **원본 패리티 갭 문서화**([19](19-parity-gap.md)) ·
>   세션 디바운스 자동 저장([20](20-session-coalescing.md)) · 쉬운 갭 4건.
>   [journal/2026-07-15](journal/2026-07-15.md)

## 1. 확정된 결정 ([10](10-decision-record.md))

| # | 영역 | 결정 |
| --- | --- | --- |
| DR-1 | 스택 | **올 러스트 단일 바이너리** — Win32(windows-rs)+커스텀 드로잉 · ADR-0001 Accepted |
| DR-2 | 예산 | 유휴 RSS ≤30MB · exe ≤10MB · 임포트=OS 인박스만 — **병합 게이트**. **개정(08-01 [ADR-0006](27-adr-0006-cloud-oauth.md))**: B3 화이트리스트 +4(winhttp·crypt32·ws2_32·bcrypt — 전부 인박스라 원칙 불변) |
| DR-3 | 배포 | **개정(07-16)**: 포터블 단일 exe **기본** + 설치형 exe **보조** 2채널(`data\` 영속·쓰기 불가 시 LOCALAPPDATA 폴백 — [21](21-distribution.md)) |
| DR-4 | 코어 | 원본 nexa-core/vfs/tree **rlib 이식**(FFI 폐지) |
| DR-5 | UX | 원본 M1 기능 패리티·디자인 규약 계승 |
| DR-6 | 라이선스 | PolyForm NC + 의존성 퍼미시브 온리 |
| DR-7 | 플러그인 | **재개정(07-26 [ADR-0005](25-adr-0005-wasm-plugins.md))**: 미리보기 플러그인 런타임 = **WASM(wasmi)** — `data\plugins\*.wasm`·fuel/메모리 격리. .NET SDK 비이관·내장(텍스트/WIC) 폴백 존치는 유지 |
| DR-8 | 외부 crate | 기본 0 지향, 건별 원장 기록(`windows`·`regex-lite`·`wasmi` 승인 — starlark은 07-26 제거) |

## 2. 예산 실측 현황 (DR-2)

| 항목 | 예산 | 최신 실측 | 시점 |
| --- | --- | --- | --- |
| B1 유휴 RSS | ≤30MB | **16.86MB**(중앙값, 3회 18/16.86/4.12 — **M5 마감 실측**: 10k·도크 정보 뷰·런처 바·유휴 300s. 활성 ~36→트림 직후 2.6MB 후 재상승 편차 큼 — 최저 4.12는 M4 수준, 재상승 원인 관찰은 β arena 회수와 공동 과제). **클라우드(X-36/37) 이후 미재측정** | 07-15 실기 |
| B2 exe 크기 | ≤10MB | **3.65MB**(`0.14.0` 릴리스 포터블 실측 — X-38/X-39 포함. `0.13.0` 3.43 대비 +0.22) | 08-02 실기 |
| P1 100k 첫 렌더 | <150ms | **115ms**(중앙값, 열거 포함 — 10k는 42ms) | 07-12 실기 |
| P2 스크롤 | 60fps(<16.7ms) | **2.1ms/프레임**(100k·200프레임 벤치) | 07-12 실기 |
| B3 임포트 DLL | OS 인박스만 | **통과** — 기존 + dwrite·combase·ole32·bcryptprimitives·shell32 + **클라우드 4종**(winhttp·crypt32·ws2_32·bcrypt — ADR-0006 개정. `scripts/budget-b3.ps1` 단일 출처) | 08-01 실기 |

## 3. 마일스톤 (상세 [MILESTONES](MILESTONES.md))

- **M0** 기반·게이트 ✅ (`0.1.0`) — 설계 문서·스캐폴딩·코어 3크레이트 이식·Win32 창·렌더 스파이크·CI·게이트 실측.
- **M1** 뷰어(★플래그십) ✅ (`0.2.0`) — 전 항목 완료 + 게이트 통과(100k 115ms·60fps·RSS).
- **M2** 셸 골격 ✅ (`0.3.0`) — 경로 바·듀얼/탭·크롬·테마·설정/세션·i18n·IME/UIA 1차·상주 규율(게이트: 듀얼·탭4 26.9MB ≤30).
- **M3** 파일 조작 ✅ (`0.4.0`) — 전송·삭제/이름변경/새로 만들기·Undo/Redo(휴지통 복원)·셸 컨텍스트 메뉴·OS 클립보드/OLE DnD·watcher + 탐색기 클릭/편집 시맨틱(게이트: 10k 유휴 300s **6.29MB** ≤30).
- **M4** 하단 패널 ✅ (`0.5.0`) — 도크·정보 뷰·미리보기·ConPTY 터미널(+상호작용 QA 시리즈)·프리즈 2건 근본 해소(게이트: 10k+터미널 상주 유휴 300s **5.07MB** ≤30).
- **M5** 마감 ✅ (`0.6.0` 발행) — M5-1(퀵 런처 바·일괄 이름변경 α+파이프라인 확장) · M5-2(릴리스 파이프라인 — 첫 태그 실행 검증 완료) · M5-3(UIA/IME 마감·서명=무서명 확정). 실기 QA 잔여.
- **포스트 M5** UX 고도화·배포 채널 정착 ✅ 진행 (`0.7.0`~**`0.13.0`**) — 툴바 SVG·ctl 컨트롤 15종·일괄 이름변경 v2·탭/전송 UX·플러그인 wasmi·안정성 감사·**클라우드 Link/Connect**(X-36/X-37).

## 4. 개발 모델 ([11](11-dev-environment.md))

- 맥 = 일상 개발(코어 test + **windows 타깃 cargo check로 UI 코드까지 타입 검증**) · Windows PC/CI = 실행·QA·예산 실측.
- **비Windows 경로 검증 필수**(08-02 교훈 — [18 §2·§4](18-build-and-test.md)): Windows `cargo test`가 green이어도 cfg 소거 경로는 미검증 → `cargo check --workspace --all-targets --target x86_64-unknown-linux-gnu`.

## 5. 다음 단계 (08-11 기준 — 상세는 [TODO](TODO.md) §7)

1. **실기 QA 잔여분 소화** — 사용자 QA가 병목. 새 기능보다 우선. **최우선 = X-40 클라우드 실검증**(`0.16.0` 배포됨 — OneDrive/Dropbox/Google Drive 동기화 폴더에서 밖의 저장·동기화가 갱신 버튼 없이 반영되는지. 로컬 NTFS는 기존 watcher로도 잡히므로 **클라우드/네트워크 경로가 진짜 검증 대상**). X-41 외관은 3라운드 QA로 확정됨.
2. **winget 0.16.0 PR 2건 심사 중**(08-11 제출 — 우리 측 조치 불요·상태 추적만): [#415214](https://github.com/microsoft/winget-pkgs/pull/415214) Portable · [#415215](https://github.com/microsoft/winget-pkgs/pull/415215) 설치형(0.8.1 등록 이후 **첫 버전 업데이트**). 카탈로그는 아직 Portable `0.13.0` · 설치형 `0.8.1`.
3. **Chocolatey 2종 승인 대기**(우리 측 조치 불요·상태 추적만): 0.8.1 **07-20 스캔 플래그 후 무이벤트**(무서명 exe 오탐 — 모더레이터 면제 필요). **후속 버전 push는 사용자 지시로 계속 제외**(0.8.1이 잠겨 있어 이중 큐 회피 — 0.15.0·0.16.0 모두 팩만 수행). 승인 시 `CHOCO_PUSH=true` 등록으로 재개. → [21 §7·§8](21-distribution.md)
4. **클라우드 배포 선행 조건 3건**(코드 작업 없음 — 본인 사용은 무관·[ADR-0006 §2-4-1](27-adr-0006-cloud-oauth.md)) — Dropbox 프로덕션 승인(가장 쉽고 효과 큼) · Entra 게시자 확인(조직 계정 동의 차단 해소 — 도메인 필요) · Google CASA(보류 — 프로덕션 게시로 7일 만료만 제거하는 절충 가능).
5. **백로그** — X-11 원본 패리티 갭 건별([19](19-parity-gap.md)) · X-2 잔여(공급자 콤보·핫 리로드·비동기 미리보기) · X-16 최적화 잔여 · X-13 2/2(브랜치 `feat/x13-launcher-crud` 보존 중) · X-14·X-15·X-24.
6. **X-33 macOS·Linux 확장** — 검토 완료([23](23-cross-platform-feasibility.md)), **착수 여부 = 사용자 결정 대기**. 진행 시 다음 액션 = 맥 렌더 스파이크(결정 아님) + DR-1/2/8 개정 ADR.
