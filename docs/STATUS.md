...
> **갱신: 2026-07-15 (KST)** — **M4 완료 → `0.5.0`(07-14)**: M4-1~3(도크·정보·미리보기·
> ConPTY 터미널)+상호작용 QA·프리즈 2건 근본 해소(아이콘 워커화·watcher OVERLAPPED).
> **07-15 UX QA 5·6차(main 직커밋)**: ① **포커스 영역 단위 강조** — `sel_bg_inactive`
> 토큰 신설, 실제 키 포커스 영역만 accent(리스트 선택색·탭 바·캐럿 테두리·도크 스트립
> [터미널]/[→] — 터미널 포커스 시 패널 전체 비활성) ② **설정 창 X-8/X-9** — VS Code식
> **즉시 적용**(저장 버튼 제거·WM_APP_PREFS_APPLY 동기 통지)+**리사이즈 창**+**원본 UI
> 재현**(사이드바 검색/카테고리 하이라이트·섹션 제목·라디오/체크박스·터미널 카테고리)
> ③ **컬럼 짝 리사이즈**(경계 드래그=좌우 동시 조절·IDC_SIZEWE 커서) ④ **도크 Info/
> Preview 라인 선택·Ctrl+C 복사** ⑤ 이름 복사를 경로 복사 바로 아래로(after_id 앵커)
> ⑥ 스플리터 강조 잔존 수정(도크 2종 재도장+WM_CAPTURECHANGED 방어).
> 테스트 158 green·clippy 0. 다음: **M5 마감**(퀵 런처·릴리스 파이프라인·접근성) ·
> X-2 Starlark · X-3 터미널 설정 잔여 · X-1 Apps 키 QA.
