...
> **갱신: 2026-09-04 (KST)** — **설정 창 본문 스크롤 재구축 + 글꼴 폴백 체인 → 릴리스 `0.19.0` → 릴리스 뒤 터미널
> 테마·복사 서식(미배포)**(사용자 QA·요청 — 상세 [journal/2026-09-04.md](journal/2026-09-04.md)):
>
...
>   실행·DLL 사이드로딩·비원자 저장·세션 동기 복원·rcPaint 무시·캐럿 타이머·watcher 클론 …) → **X-52~X-57**.
>   테스트가 잡은 3건(VT 오버플로·IL/DL 소진·`get_runs` 언더플로)+압축 이름 절단 패닉 = **즉시 수정**.
>   **즉시 조치 4건 완료**(사용자 지시): 덮어쓰기 스테이징 교체 · 드롭 거부+복귀 · ConPTY 전체 경로 · CFG/CET/
>   DEPENDENTLOADFLAG+매니페스트 → 재판정 [`20260904-174325`](audit/20260904-174325/summary.md) PASS=8 FAIL=0 WARN=0 SKIP=2. 이어서 플러그인 벽시계·과금·브레이커(`e846968`)와 설정 저장 원자화까지 → **§7-1 즉시 6건 전부 완료**.
