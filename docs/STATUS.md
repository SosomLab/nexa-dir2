...
> **갱신: 2026-08-01 8차 (KST)** — **X-37 7~13차 클라우드 실사용 완성
> (사용자 QA 연속 17건 — `feat/x36-cloud-connections` 22커밋, 병합 대기)**:
> 6차까지가 기능 배치였다면 이후는 **실기 QA가 결함을 뽑아낸** 단계 —
> 보고 17건이 전부 진짜 결함, 다수가 **한 증상 = 두 원인**.
> **Google 완성**(`drive.file`은 탐색기로 성립 불가 → `drive` 전체·
> **조용한 실패의 진범 = Google이 데스크톱 앱에도 요구하는
> `client_secret`**·계정명은 Drive about·인증 실패 **모달 승격**·
> **Link/Connect 용어 분리**) · **진행 표시**(폴더 열기 = 플레이스홀더 +
> 플로팅 배지 · 전송 = 로컬 복사 창 재사용) · **로딩 무한 5건**
> (`::PC::` 매칭 누락·ID 캐시 휘발·**실패 미캐시로 무한 재시도**·
> 비활성 탭·경로 바 조각의 루프 안 `?` 조기 종료) · **계정 간 복사**
> (대상 폴더 미생성·**`conflictBehavior: replace`가 기존 폴더를 지울 수
> 있던 것**·건수를 바이트로 쓰던 표시·Google 단일 multipart라
> **36MB가 끝나야 반영**되던 것 → resumable 청크·**100%에서 멈추고
> [취소] 무반응**[취소 판정이 진행 통지 안에만 있었다] → 타이머 분리) ·
> **UI**([홈] 버튼 신설·U+EA8A 13 DIP·**컬럼 폭 동기 기동 시 정렬**) ·
> **Dropbox 실사용 5건**(scope 읽기 전용·리디렉션 정확 일치·
> **`is_api()`가 account 유무를 봐서 연결이 통째로 누락**·한글이
> **헤더 ASCII 제약**에 걸림·**`json_str`의 `\uXXXX` 미해석이
> HTTP 409까지 연쇄**). **254 green·clippy 0·3.44MB(B2)·B3 통과**.
> 사용자 실기 확인: 3사 연결·탐색·업/다운로드·계정 간 복사·한글 이름.
> [journal/2026-08-01.md](journal/2026-08-01.md).
>
> **직전(08-01 7차)** — **X-37 2~6차 클라우드 실사용화
> (사용자 QA 연속 — `feat/x36-cloud-connections`, 실기 QA 중)**:
> 인증만 되고 아무것도 못 하던 상태 → **3사 탐색 + OneDrive/Dropbox
> 전 기능**. 2차 탐색(`::CLOUD:` 센티널·워커 캐시·UI 네트워크 금지) ·
> 기동 순서 픽스 · **3차 다운로드**(HTTP가 이진을 UTF-8로 손상하던
> 결함 → `Vec<u8>`·더블클릭 열기·DnD) · **4차 쓰기**(scope
> `Files.ReadWrite` **재로그인 1회 필요**·**상태 코드 미검사 결함**
> 수정·업로드 세션 청크) · **5차 표시명**(센티널 노출 4곳 라벨화·
> **Alt+↑ 무동작 해소**·폴더 재귀·클라우드 간 서버 사이드 복사) ·
> **6차 Dropbox 전체 신규 + Google 하위 폴더**. **248 green·clippy 0·
> 3.38MB(B2)·B3 통과**. **QA 대기**: 재연결(쓰기 권한) 후 업로드·폴더
> 복사·클라우드 간 복사. [journal/2026-08-01.md](journal/2026-08-01.md).
>
> **직전(08-01 6차)** — **client_id 소유 모델 정정 —
