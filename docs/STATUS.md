...
> **M4-1 하단 도크 완료**(M4 착수): InfoDock 위젯(패널별 1개 — 원본 대원칙)·정보 뷰(원본
> DockInfo — 다중=개수/단일=속성/없음=현재 폴더)·Ctrl+` 토글·높이 드래그(비율 영속)·
> settings 저장/복원. 테스트 140 green·B3 무변·실기 QA 대기.
> 이전: **M3 완료 `0.4.0`**(게이트: B1 10k 유휴 300s 6.29MB ≤30).
> 다음: **M4-2 내장 미리보기**(텍스트·이미지 — WIC 디코딩, 도크 종류 스왑).
...
- **M4** 하단 패널 🚧 — M4-1 ✅(도크·정보 뷰·토글·드래그·영속) · 미리보기·터미널 ☐.
- M5 마감 — ☐.
...
1. ~~M0~~ ✅ `0.1.0` · ~~M1~~ ✅ `0.2.0` · ~~M2~~ ✅ `0.3.0` · ~~M3~~ ✅ `0.4.0` · ~~M4-1~~ ✅ (07-13).
2. **M4-2 내장 미리보기**(P1, 중) — 텍스트·이미지(WIC 디코딩), 도크 종류 스왑(원본 docs/35 차용·DR-7).
3. M4-3 ConPTY 터미널(대). → [02](02-roadmap.md)
