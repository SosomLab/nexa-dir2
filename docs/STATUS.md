...
> **M4-2 내장 미리보기 완료**: 도크 [정보|미리보기] 종류 스트립(클릭 전환)·텍스트(16KB·
> 이진 판정)·이미지(**WIC 내장 — 플러그인 아님, DR-7**·Fant 스케일·비율 유지·캐시 8건·
> CoCreateInstance 지연 활성화=임포트 무변). 독립 검증 예제 examples/preview_image.rs.
> 이전: M4-1 도크(정보 뷰·Ctrl+` 토글·높이 드래그·영속) · M3 완료 `0.4.0`.
> 다음: **M4-3 ConPTY 터미널**(대 — 원본 VtScreen·docs/37 이식) → M4 마감.
...
- **M4** 하단 패널 🚧 — M4-1·2 ✅(도크·정보 뷰·미리보기[텍스트·WIC 이미지 — 내장, DR-7]) · 터미널 ☐.
- M5 마감 — ☐.
...
1. ~~M0~~ ✅ `0.1.0` · ~~M1~~ ✅ `0.2.0` · ~~M2~~ ✅ `0.3.0` · ~~M3~~ ✅ `0.4.0` · ~~M4-1·2~~ ✅ (07-13).
2. **M4-3 ConPTY 터미널**(P1, 대) — 원본 VtScreen·docs/37 이식(도크 종류 추가) → M4 마감.
3. M5 마감(퀵 런처·릴리스 파이프라인·접근성 마감). → [02](02-roadmap.md)
