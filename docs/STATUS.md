...
> **M4-3 ConPTY 터미널 완료 → M4 전 항목 구현 완료**: nexa-term rlib(VtScreen — SGR·CSI·
> DECSTBM·스크롤백·전각, 테스트 9)·ConPTY(pwsh→cmd 폴백·세대 가드)·도크 [터미널] 종류
> (Consolas 모노 그리드·클릭 포커스·VT 키·아무 키 재시작). 테스트 148 green·B2 0.69MB·
> B3 무변·실기 QA 대기. + ADR-0004(미리보기 Starlark 플러그인 — DR-7 개정, 백로그 X-2).
> 다음: **M4 마감 게이트**(B1 실측 — 터미널 상주분 확인) → M5 마감(릴리스 파이프라인 등).
...
- **M4** 하단 패널 🚧 — **M4-1~3 전 항목 구현 완료**(도크·정보 뷰·미리보기·ConPTY 터미널) · 마감 게이트(B1 실측) 잔여.
- M5 마감 — ☐.
...
2. **M4 마감 게이트** — 실기 QA(터미널) + B1 유휴 재실측(터미널 상주분) → `0.5.0` 태그.
3. M5 마감(퀵 런처·릴리스 파이프라인·접근성) · X-2 Starlark 플러그인. → [02](02-roadmap.md)
