...
> **갱신: 2026-07-13 (KST)** — **M2-7 IME/UIA 1차 완료 → M2 셸 골격 완료 `0.3.0`**(원본 NFR-A1):
> IME 조합 창 캐럿 배치(경로바 편집·WM_IME_*) + UIA 서버측 프로바이더(불변 스냅샷 — List/ListItem·
> SelectionItem·FocusChanged). 실기 = .NET UIA 클라이언트로 리스트/항목/선택 조회 검증.
> M2 게이트 통과(듀얼·탭4 상주 26.9MB ≤30). 다음: **M3 파일 조작**(M3-1 `nexa-ops` 전송 엔진, P0).
...
1. ~~M0~~ ✅ `0.1.0` · ~~M1~~ ✅ `0.2.0` · ~~M2~~ ✅ `0.3.0` (07-13) — 셸 골격 완료.
2. **M3-1 `nexa-ops` 전송 엔진**(P0, 대) — 복사/이동·진행률·충돌·취소(원본 TransferPathsInto 스펙).
3. M3-2 삭제/이름변경/새로 만들기(P0) → M3-3 Undo/Redo. → [02](02-roadmap.md)
