...
> **갱신: 2026-07-12 (KST)** — **M2-6 i18n 완료**(원본 docs/42): 내장 en/ko + exe 옆 `data\lang\`
> 키 단위 오버라이드(properties 스타일·crate 0)·**동적 전환**(언어 라디오 → 테이블 스왑+메뉴/컬럼
> 재구성+재그리기, 재시작 불요)·"system"=OS 언어 추종·settings `lang` 영속. 실기 타이틀 ko/en 검증.
> 다음: **M2-8 상주 규율**(P0 — RSS 트림 감시 항목 해소, 99.5MB 장기 상주 관측) 또는 **M2-7 IME/UIA**(P1).
...
- **M2** 셸 골격 🚧 — M2-1 α~M2-6 ✅(경로 바·듀얼/탭·크롬·테마·설정/세션 영속·i18n) · IME/UIA·상주 규율 ☐.
...
1. ~~M0~~ ✅ `0.1.0` · ~~M1~~ ✅ `0.2.0` · ~~M2-1 α~6~~ ✅ (07-12) — 경로 바·듀얼/탭·크롬·테마·설정/세션·i18n.
2. **M2-8 상주 규율**(P0) — 유휴 트림(RSS 여유 2MB + 99.5MB 장기 상주 관측 해소)·상주 RSS ≤30MB 게이트.
3. M2-7 IME(한글)/UIA 1차(P1). → [02](02-roadmap.md)
