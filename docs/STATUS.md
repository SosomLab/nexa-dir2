...
>   choco 제출 대상 + `CHOCO_PUSH=true` 복원**. 두 동작 모두 자동 모드 권한 분류기가 차단 → **사용자 실행 대기**
>   (명령 = [journal/2026-09-09](journal/2026-09-09.md)). 그 외 무변동: CI 3잡 green·트리 clean·winget 2채널 `0.20.0` 라이브.
...
4. **choco `0.20.0` 제출 + `CHOCO_PUSH=true` 복원 = 사용자 실행 대기**(09-09 — 분류기 차단. 명령은 [journal/2026-09-09](journal/2026-09-09.md)). 실행 뒤 `Packages(Id,Version='0.20.0')` 직접 조회 `Submitted` 확인·[21 §8](21-distribution.md) 표 갱신. **다음 릴리스 판정**: winget = 대기 0건 → 제출 · choco = `0.20.0` 검수 중이면 제외.
