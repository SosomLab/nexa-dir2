...
4. **choco `0.20.0` 두 패키지 `Submitted`**(09-09 02:51Z — [run 34304846007](https://github.com/SosomLab/nexa-dir2/actions/runs/34304846007)) · **`CHOCO_PUSH=true`**(복원 완료). **다음 릴리스 판정**: winget = 대기 0건 → 제출 · choco = `0.20.0`이 아직 검수 중이면 **`false`로 내려 제외**, `Approved`면 태그 push가 자동 게시. 승인 확인 = `Packages(Id,Version='0.20.0')` 직접 조회.
