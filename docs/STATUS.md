...
> **M4 완료 → `0.5.0`(07-14)**: M4-1~3(도크·정보·미리보기·ConPTY 터미널) + 실기 QA
> 시리즈 — 셀 단위 렌더(한글 열 정렬)·드래그 선택·스크롤백 휠·Ctrl+C/V·캐럿(세로바·
> 깜빡임)·term_font 설정. **프리즈 2건 근본 해소**: ① 파일별 아이콘 SHGetFileInfoW
> 워커화(Defender/OneDrive) ② **watcher drop CloseHandle 직렬화**(OVERLAPPED 재구현 —
> 12,009ms→4ms 실측). + 편집 필드 Ctrl+C/X/V·리네임 지연·휠 hover 라우팅·.lnk 숨김.
> **B1 게이트 통과**(터미널 상주 300s 5.07MB ≤30)·151 green·B2 0.73MB·B3 무변.
> 다음: **M5 마감**(퀵 런처·릴리스 파이프라인·접근성) · X-4 펼침 유지(F18) · X-2 Starlark.
...
2. **M5 마감**(퀵 런처·릴리스 파이프라인·접근성 마감·셸 메뉴 후속) — 착수 전 [02](02-roadmap.md) 세부 확정.
3. 백로그: **X-4 펼침 유지(원본 F18 — 제안 승인 대기)** · X-2 Starlark 플러그인 · X-3 터미널 설정 잔여 · X-1 Apps 키 QA.
