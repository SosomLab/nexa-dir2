...
> **갱신: 2026-07-12 (KST)** — **M2-4 테마 시스템 완료**(원본 docs/39): 시스템(OS 추종·WM_SETTINGCHANGE
> 실시간)/라이트/다크 — 보기 메뉴 라디오·F6 순환·다크 타이틀바(DWM) 동기. 실기 픽셀 검증(다크 #1F242B ↔
> 라이트 #F5F7FA·시스템 추종). 모드 영속은 M2-5로.
> 다음: **M2-5 설정/세션 영속**(P0) — data\ 설정·탭/창 상태 저장·복원(원본 docs/34·40·43).
...
- **M2** 셸 골격 🚧 — M2-1 α~M2-4 ✅(경로 바·듀얼/탭·크롬·테마) · 설정/세션·i18n·IME/UIA·상주 규율 ☐.
...
1. ~~M0~~ ✅ `0.1.0` · ~~M1~~ ✅ `0.2.0` · ~~M2-1 α~4~~ ✅ (07-12) — 경로 바·듀얼/탭·크롬·테마.
2. **M2-5**: 설정/세션 영속(P0) — exe 옆 `data\` settings/session(테마 모드·필터·탭·창), 원본 docs/34·40·43 차용.
