...
> **갱신: 2026-07-12 (KST)** — **M2-5 설정/세션 영속 완료**(원본 docs/34·40·43): exe 옆 `data\`에
> settings.txt(테마·필터·스플리터)·session.txt(패널별 탭·활성) — key=value 텍스트(crate 0)·원자적 쓰기
> (tmp→rename)·기동 로드/종료 저장. 실기 재실행 복원 검증(라이트 테마·탭 2/2·활성 경로).
> 다음: **M2-8 상주 규율**(P0 — RSS 트림 감시 항목 해소) 또는 **M2-7 IME/UIA**(P1).
...
- **M2** 셸 골격 🚧 — M2-1 α~M2-5 ✅(경로 바·듀얼/탭·크롬·테마·설정/세션 영속) · i18n·IME/UIA·상주 규율 ☐.
...
1. ~~M0~~ ✅ `0.1.0` · ~~M1~~ ✅ `0.2.0` · ~~M2-1 α~5~~ ✅ (07-12) — 경로 바·듀얼/탭·크롬·테마·설정/세션.
2. **M2-8 상주 규율**(P0) — 유휴 트림(RSS 여유 2MB 감시 항목 해소)·상주 RSS ≤30MB 게이트.
3. M2-7 IME(한글)/UIA 1차(P1) · M2-6 i18n(P2 — 동적 전환 설계 확정). → [02](02-roadmap.md)
