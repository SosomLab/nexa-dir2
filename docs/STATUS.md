...
> **갱신: 2026-08-23 (KST)** — **X-42 가상 파일 붙여넣기 + 릴리스 채널 제출 규칙**:
>
...
1. **실기 QA 잔여분 소화** — 사용자 QA가 병목. 새 기능보다 우선. **최우선 = X-40 클라우드 실검증**(`0.16.0` 배포됨 — OneDrive/Dropbox/Google Drive 동기화 폴더에서 밖의 저장·동기화가 갱신 버튼 없이 반영되는지. 로컬 NTFS는 기존 watcher로도 잡히므로 **클라우드/네트워크 경로가 진짜 검증 대상**) + **X-42 가상 파일 붙여넣기**(08-23 구현·미배포 — 맥 RDP에서 복사→앱 붙여넣기: 단일/다중/폴더/대용량/한글 이름. Outlook 첨부·zip 내부도 같은 경로). X-41 외관은 3라운드 QA로 확정됨.
2. **winget 0.16.0 PR 2건 심사 중**(08-11 제출 — 우리 측 조치 불요·상태 추적만): [#415214](https://github.com/microsoft/winget-pkgs/pull/415214) Portable · [#415215](https://github.com/microsoft/winget-pkgs/pull/415215) 설치형(0.8.1 등록 이후 **첫 버전 업데이트**). 카탈로그는 아직 Portable `0.13.0` · 설치형 `0.8.1`. **다음 릴리스부터 [21 §8 채널 제출 규칙](21-distribution.md) 적용**(대기 중이면 제외·없으면 제출 — 08-23 사용자 지시로 상시 규칙화).
3. **Chocolatey 2종 승인 대기**(우리 측 조치 불요·상태 추적만): 0.8.1 **07-20 스캔 플래그 후 무이벤트**(무서명 exe 오탐 — 모더레이터 면제 필요). **후속 버전 push는 채널 제출 규칙에 따라 계속 제외**(0.8.1이 잠겨 있어 이중 큐 회피 — 0.15.0·0.16.0 모두 팩만 수행). 승인 시 `CHOCO_PUSH=true` 등록으로 재개. → [21 §7·§8](21-distribution.md)
