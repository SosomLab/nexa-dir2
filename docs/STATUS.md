...
>
> - **깜박임·트랙패드·스크롤바 3건 해소**(`f5b484a`): 휠마다 전 컨트롤을 파괴·재생성하던 구조가
>   진범 → **본문 컨테이너 분리 + 자식 일괄 이동**(재구성 없음) · 휠 잔량 누적 = **픽셀 단위
>   트랙패드 스크롤** · **오버레이 스크롤바**(그리드 규약 + 반투명·단계 페이드·호버 두꺼움 —
>   사용자 지정 방식) · 더블버퍼로 **썸 블링크 해소**. 실측 교훈 = WS_EX_COMPOSITED 위에선 GDI+
>   알파가 안 그려진다. **깜박임 소멸 여부는 사용자 실기 확인 대기**(정지 캡처로는 못 잡음).
...
> - **트리 하이라이트 잔존 픽스**(사용자 QA — 컨테이너 분리로 드러난 잠복 결함): rebuild 끝 트리 무효화.
