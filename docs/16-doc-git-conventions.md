...
> **목적**: 이 저장소에서 검증된 **문서 체계 + git 작업 규약**을 다른 프로젝트에 그대로 적용하기 위한 **휴대용 표준 문서**.
> **사용법**: 아래 [§0 지시문](#0-지시문-복사해서-붙여넣기)을 새 프로젝트의 AI 세션에 **그대로 붙여넣거나**, 이 파일을 새 저장소 `docs/`에 복사한 뒤 `{{PROJECT}}` 자리표시자만 바꾼다.
> 본 저장소의 적용 실체: [README](README.md)(색인) · [15 개발 방법론](15-dev-methodology.md)(개발 규율) · [18 빌드&테스트](18-build-and-test.md)(SSOT 예시).
