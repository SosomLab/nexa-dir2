...
| X-10 | **설정 창 VS Code 심화**(사용자 분석 요청 07-15 — 스크린샷 기준): ~~① 검색 시 좌측 카테고리 매치 필터~~ ✅(07-15 — 계층 트리 개편+라벨/상세 매치 필터+드릴다운+검색 유지 탐색+✕ 지우개. **매치 수 "(N)" 표기는 잔여**) ② 다중 토큰 AND 매칭+결과 수 표기 ③ 항목 **설명 문장** 렌더(제목 아래) ④ **수정됨 표시**(기본값과 다른 항목 좌측 세로 바+기본값 복원) ⑤ 제목 "카테고리: 항목" 접두 규약 | P3 | 중 | X-9 | 🚧 (②~⑤·①매치 수 잔여) |
