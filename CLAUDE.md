...
현 단계: **포스트 M5 — UX 고도화 + 배포 채널 정착**. M0(`0.1.0`)~M5(`0.6.0`) 완료, 최신 릴리스 **`0.16.0`**(08-11 — X-40 외부·클라우드 변경 자동 갱신·X-41 도구 모음 크기. **winget 2채널 배포 완료**[08-06 병합] + **0.16.0 PR 2건 심사 중**, Chocolatey 2종은 0.8.1 모더레이션에 잠긴 채라 **사용자 지시로 push 제외** — [21 §7·§8](docs/21-distribution.md)).
...
2. **winget 0.16.0 PR 2건 심사 중**(08-11 제출·우리 측 조치 불요·상태만 추적): [#415214](https://github.com/microsoft/winget-pkgs/pull/415214) Portable · [#415215](https://github.com/microsoft/winget-pkgs/pull/415215) 설치형(0.8.1 등록 이후 첫 버전 업데이트). 카탈로그는 아직 Portable 0.13.0 · 설치형 0.8.1.
3. **Chocolatey 2종 승인 대기**(우리 측 조치 불요·상태만 추적 — 08-11 기준): 0.8.1 두 패키지 모두 **큐 대기가 아니라 07-20 바이러스 스캔 플래그 후 무이벤트**(무서명 exe 오탐 — 모더레이터 면제 필요). **후속 버전 push는 사용자 지시로 계속 제외**(0.8.1이 잠겨 있어 이중 큐를 만들지 않는다) — 승인 시 저장소 변수 `CHOCO_PUSH=true` 등록으로 재개(현재 미등록). → [21 §7·§8](docs/21-distribution.md)
4. **클라우드 배포 선행 조건 3건**(X-37 잔여 = 코드 아님·본인 사용은 무관 — [ADR-0006 §2-4-1](docs/27-adr-0006-cloud-oauth.md)): Dropbox 프로덕션 승인(무료·수일 — 가장 쉽고 효과 큼) · Entra 게시자 확인(조직 계정 동의 차단 해소 — 커스텀 도메인 필요) · Google CASA(보류 — 프로덕션 게시로 7일 만료만 제거하는 절충 가능).
5. **백로그 진행** — [docs/TODO.md](docs/TODO.md) §7: X-11 원본 패리티 갭 건별([19](docs/19-parity-gap.md)) · X-2 플러그인(wasmi 전환 완료 — 잔여 콤보·핫 리로드) · X-16 최적화 잔여 · X-13 2/2.
6. **X-33 macOS·Linux 확장** — 검토 완료([23](docs/23-cross-platform-feasibility.md)), **착수 여부는 사용자 결정 대기**. 진행 시 다음 액션 = 맥 렌더 스파이크(결정 아님) + DR-1/2/8 개정 ADR(신규 번호 = ADR-0007).
