...
3. **배포 채널 = 상태 추적**(09-08 실측): **winget `0.19.0` PR 2건 09-04 당일 병합**([#429155](https://github.com/microsoft/winget-pkgs/pull/429155)·[#429156](https://github.com/microsoft/winget-pkgs/pull/429156)) → **`0.20.0` 2건 제출 → 당일 MERGED**([#431268](https://github.com/microsoft/winget-pkgs/pull/431268)·[#431269](https://github.com/microsoft/winget-pkgs/pull/431269) — winget 2채널 = 최신 릴리스와 동기, 다음 릴리스 winget 제출 대상) · **Chocolatey 2종 `0.8.1` 승인**(07-20 스캔 플래그 후 44일 — 모더레이터 면제) → `0.18.1`만 제출(09-02 `resubmit-chocolatey` dispatch) → **09-08 `Ready`**(자동 검증·verification 통과·스캔 1~5 탐지 플래그 = 승인 차단 아님·모더레이터 검토 대기). → [21 §7·§8](docs/21-distribution.md)
4. **`CHOCO_PUSH`는 지금 `false`**(09-04 등록·09-08 유지 — `0.20.0`도 choco 제외: `0.18.1`이 `Ready` 검수 중). **`0.18.1` 승인 확인 시 `true`로 복원**하고 그 시점 최신 버전만 제출(중간 버전 생략 — [21 §8](docs/21-distribution.md)).
