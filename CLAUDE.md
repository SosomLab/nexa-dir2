...
3. **배포 채널 = 상태 추적**(09-08 실측): **winget `0.19.0` PR 2건 09-04 당일 병합**([#429155](https://github.com/microsoft/winget-pkgs/pull/429155)·[#429156](https://github.com/microsoft/winget-pkgs/pull/429156)) → **`0.20.0` 2건 제출 → 당일 MERGED**([#431268](https://github.com/microsoft/winget-pkgs/pull/431268)·[#431269](https://github.com/microsoft/winget-pkgs/pull/431269) — winget 2채널 = 최신 릴리스와 동기, 다음 릴리스 winget 제출 대상) · **Chocolatey 2종 `0.8.1` 승인**(07-20 스캔 플래그 후 44일 — 모더레이터 면제) → `0.18.1`만 제출(09-02 `resubmit-chocolatey` dispatch) → **09-09 `Approved` 확인**(virtualex 09-08 — 제출 6일). → [21 §7·§8](docs/21-distribution.md)
4. **사용자 실행 대기 2건**(09-09 — 자동 모드 권한 분류기가 `gh variable set`·`gh workflow run`을 차단): ① `gh variable set CHOCO_PUSH -R SosomLab/nexa-dir2 --body true` ② `gh workflow run resubmit-chocolatey.yml -R SosomLab/nexa-dir2 -f version=0.20.0 -f confirm=yes`(빌드 없음·태그 무변 — 두 패키지 함께). 실행 뒤 choco `0.20.0` `Submitted` 확인·21 §8 표 갱신. 그 뒤 `CHOCO_PUSH`는 `true` 상태 = 다음 태그 push가 choco도 자동 게시(단 `0.20.0` 검수 중이면 규칙상 제외 — 그때는 `false`로 다시 내린다).
