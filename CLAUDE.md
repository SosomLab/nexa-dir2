...
현 단계: **포스트 M5 — UX 고도화 + 배포 채널 정착**. M0(`0.1.0`)~M5(`0.6.0`) 완료, 최신 릴리스 **`0.20.0`**(09-08 — **X-49 터미널 테마**[라이트 팔레트 GitHub Light·스킴 15종·선택자 system/dark/light/스킴 + 모드별 기본]·**X-50 터미널 복사 서식**[HTML·RTF 4택]·**X-51 정규 점검 1차 즉시 조치 6건**[덮어쓰기 스테이징·드롭 거부·ConPTY 전체 경로·CFG/CET+매니페스트·플러그인 벽시계·설정 저장 원자화 + 파서 결함 4건. 점검 SSOT = docs/29 + `docs/audit/<회차>/`]. 채널 = winget 2건 당일 병합·choco는 릴리스 시점 제외[`0.18.1` `Ready`] → **09-09 `0.18.1` 승인 확인 → `0.20.0` choco 두 패키지 `Submitted` + `CHOCO_PUSH=true` 복원**(§7-4). 그 전 `0.19.0` = 설정 창 스크롤 재구축·글꼴 폴백 체인·파일 목록 오버레이 바[winget 2건 09-04 당일 병합]. `0.18.1` = 설치형 VERSIONINFO 보강 = Defender ML 오탐 대응 재배포. `0.18.0` = **X-46 압축 파일 미리보기**[그리드 창·암호 입력·플러그인 ABI v2 — [28](docs/28-archive-preview.md)] + **미리보기 플러그인 2종 동봉 배포**[markdown.wasm·archive.wasm — 포터블 zip·설치본·플러그인 zip. [21 §5-2](docs/21-distribution.md)]. **winget 2채널 `0.18.1` 라이브**[#423330·#423331 병합]·**Chocolatey 2종 승인**[0.8.1 — 07-20 플래그 후 44일] → 09-02 `CHOCO_PUSH=true` 등록·**`0.18.1` 두 패키지 제출·검수 중** — [21 §7·§8](docs/21-distribution.md)).
...
- **정규 점검 SSOT = [docs/29](docs/29-audit-checklist.md)** — 릴리스마다 `pwsh scripts/audit.ps1`(자동 판정) + §3~§8 리뷰 + §9 회차 기록. 발견은 TODO 항목화.
...
## 7. 다음 단계 (2026-09-08 갱신)

> M0~M5는 전부 완료(`0.1.0`~`0.6.0`), 이후 포스트 M5 UX 고도화로 `0.20.0`까지 릴리스됨.
> 아래는 **지금 열려 있는 것**만. 최신 현황은 항상 [docs/STATUS.md](docs/STATUS.md).

1. **점검 1차 잔여 처리**(09-04 — 회차 보고 [§7-2 단기](docs/audit/20260904-165351/README.md)): **즉시 6건은 완료**. 다음 = X-54 paint rcPaint·캐럿 타이머(백그라운드 CPU) → X-54 watcher 클론·`index_of_path` 해시 → X-53 세션 지연 복원·저장 디바운스 → X-52 하위 트리 오류 격리·O(N²) 진행 → X-56 워커 이동·OUT_CAP·능력별 링커 → X-57. **정책 결정(사용자)**: 코드 서명·출처 증명 · OAuth 시크릿 하드코딩 · 플러그인 출처 · 단일 인스턴스 · overflow-checks.
2. **실기 QA 잔여분 소화** — 사용자 QA가 병목. 새 기능보다 우선. **최신 대기분(`0.20.0` 배포분, 09-08)** = **X-49 터미널 테마**(라이트 선택 블록·F6 즉시 재색칠·다크 앱+라이트 스킴·콤보 즉시 반영/영속) **+ X-50 터미널 복사 서식**(both → Word/Outlook 붙여넣기 색·글꼴 유지·메모장 평문·기본 평문만) **+ 즉시 조치 6건 회귀**(덮어쓰기 취소 시 원본 보존·전송 중 드롭 거부 안내·설정 저장·플러그인 격리 문구). 그 앞 대기분(`0.19.0` 배포분) = 설정 창 스크롤 재구축(깜박임·트랙패드·오버레이 바·썸 블링크) · 파일 목록 오버레이 바(세로+가로·축별 독립) · 글꼴 폴백 체인(두부 방지 — X-48) · 트리 하이라이트 픽스. 그 앞 대기분 = **X-46 압축 파일 미리보기 + 플러그인 동봉**(08-24 `0.18.0` 배포 — 실제 zip/7z/rar/cab/iso·암호 zip 입력창·틀린 암호 재시도·한글 이름 구형 zip·대용량 그리드 정렬/복사·**포터블 zip 해제 후 exe 옆 plugins\ 자동 인식**·설치본 `{app}\plugins`) + **X-42 가상 파일 1·2차**(RDP 붙여넣기+워커·undo·DnD·클라우드) + **X-43 빈 폴더 글리프+β** + **X-44 간헐 무갱신 1~5차**(핵심 확인 = 클라우드 생성 → 로컬 반영 순간 비활성에서 즉시 표시[5차 셸 통지] — 플레이스홀더 무통지는 실측 확정) + **X-45 항상 맨 위에**(전부 08-23 구현·미배포·재시작 반영) + X-40 클라우드 실검증.
...
3. **배포 채널 = 상태 추적**(09-08 실측): **winget `0.19.0` PR 2건 09-04 당일 병합**([#429155](https://github.com/microsoft/winget-pkgs/pull/429155)·[#429156](https://github.com/microsoft/winget-pkgs/pull/429156)) → **`0.20.0` 2건 제출 → 당일 MERGED**([#431268](https://github.com/microsoft/winget-pkgs/pull/431268)·[#431269](https://github.com/microsoft/winget-pkgs/pull/431269) — 09-10 카탈로그·로컬 `winget show` 라이브 확인. winget 2채널 = 최신 릴리스와 동기, 다음 릴리스 winget 제출 대상) · **Chocolatey 2종 `0.8.1` 승인**(07-20 스캔 플래그 후 44일 — 모더레이터 면제) → `0.18.1`만 제출(09-02 `resubmit-chocolatey` dispatch) → **09-09 `Approved` 확인**(virtualex 09-08 — 제출 6일). → [21 §7·§8](docs/21-distribution.md)
4. **`CHOCO_PUSH=true`**(09-09 복원 — 분류기 차단 뒤 사용자 명시 승인으로 실행) · **choco `0.20.0` 두 패키지 검수 중**(09-09 `resubmit-chocolatey` run 34304846007 — 빌드 없음·태그 무변 → 09-10 실측 검증·verification 통과·스캔 `Pending`). 다음 릴리스: `0.20.0`이 아직 검수 중이면 규칙상 choco 제외 = `false`로 다시 내림, `Approved`면 태그 push가 자동 게시. 교훈: `gh variable set`·`gh workflow run … confirm=yes`는 자동 모드에서 차단됨 — 명령을 제시하고 사용자 승인 뒤 실행.
