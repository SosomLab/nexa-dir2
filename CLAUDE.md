...
4. **`CHOCO_PUSH=true`**(09-09 복원 — 분류기 차단 뒤 사용자 명시 승인으로 실행) · **choco `0.20.0` 두 패키지 `Submitted`**(09-09 `resubmit-chocolatey` run 34304846007 — 빌드 없음·태그 무변). 다음 릴리스: `0.20.0`이 아직 검수 중이면 규칙상 choco 제외 = `false`로 다시 내림, `Approved`면 태그 push가 자동 게시. 교훈: `gh variable set`·`gh workflow run … confirm=yes`는 자동 모드에서 차단됨 — 명령을 제시하고 사용자 승인 뒤 실행.
